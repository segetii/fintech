...
def run_benchmark():
    ...
    log("=" * 80)
    log()
    log(f"  {'Experiment':<35s} {'Mean AUC':>10s} {'D vs Base':>10s} {'Verdict':>10s}")
    log("  " + "─" * 70)
    ...
    log()
    log("=" * 80)
    log("  AUGMENTATION VALUE (D AUC when added to baseline)")
    log("=" * 80)
    log()
    ...
    log(f"  {'Operator':<25s} {'AUC':>8s} {'D AUC':>10s} {'Verdict':>10s}")
    log("  " + "─" * 55)
