...
const HIGH_IMPACT_LABELS = ['FRAUD', 'SANCTIONED', 'PEP', 'FALSE_NEGATIVE'];
const DUAL_CONTROL_LABELS = ['FRAUD', 'SANCTIONED'];
// ═══════════════════════════════════════════════════════════════════════════
...
export class LabelSafeguards {
     ...
     */
    determineImpact(labelType, confidence) {
        if (HIGH_IMPACT_LABELS.includes(labelType))
            return 'HIGH';
        if (confidence > 0.9)
            return 'MEDIUM';
        return 'LOW';
     ...
     */
    getRequiredApprovals(labelType, impact) {
        if (DUAL_CONTROL_LABELS.includes(labelType))
            return 2;
        if (impact === 'HIGH')
            return 2;
        if (impact === 'MEDIUM')
     ...
     */
    async submitLabel(user, submission) {
        // RBAC check
     ...
     */
    async approveLabel(user, labelId, decision, comments, ipAddress) {
    ...
    getAuditLog(user, filters) {
        ...
        if (filters?.labelId)
            log = log.filter(l => l.labelId === filters.labelId);
        if (filters?.userId)
    ...
    computeProvenanceHash(submission, approvals) {
        const data = {
            submission: {
                id: submission.id,
    ...
    logAction(action, labelId, userId, details) {
        this.auditLog.push({
            action,
            labelId,
            userId,
    ...
    getStats() {
        ...
        return {
            ...
            approved24h: records.filter(r => r.status === 'APPROVED' &&
                r.approvals.some(a => a.approvedAt > last24h)).length,
            avgApprovalTime: this.calculateAvgApprovalTime(records),
            dualControlPending: records.filter(r => r.status === 'PENDING' &&
                DUAL_CONTROL_LABELS.includes(r.submission.labelType)).length,
        };
    }
    calculateAvgApprovalTime(records) {
...
// Singleton
let safeguardsInstance = null;
export function getLabelSafeguards() {
