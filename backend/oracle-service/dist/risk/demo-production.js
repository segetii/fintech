 ...
 */
import { ExplainabilityService, RiskExplainer, explainRiskDecision } from './explainability/index.js';
// ═══════════════════════════════════════════════════════════════════════════════
...
async function demoExplainer() {
    ...
    const explanation = explainer.explain(sampleRequest.riskScore, sampleRequest.features, sampleRequest.graphContext, sampleRequest.ruleResults);
    printExplanation(explanation);
}
// ═══════════════════════════════════════════════════════════════════════════════
...
async function demoConvenience() {
    ...
    console.log('═'.repeat(80) + '\n');
    const explanation = explainRiskDecision(0.73, { amountEth: 847.5, dormancyDays: 190 }, { hopsToSanctioned: 2 }, [{ ruleType: 'LAYERING', triggered: true, confidence: 0.85 }], 2500);
...
function printExplanation(explanation) {
    ...
    if (explanation.typologyMatches?.length > 0) {
        ...
        console.log('─'.repeat(80));
        explanation.typologyMatches.forEach((match) => {
    ...
    explanation.factors.slice(0, 8).forEach((factor) => {
