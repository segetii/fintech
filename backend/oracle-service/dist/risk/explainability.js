...
export class RiskExplainer {
    ethPriceUsd;
    constructor(ethPriceUsd = 2500) {
        this.ethPriceUsd = ethPriceUsd;
    }
    /**
     * Generate a complete explanation for a risk decision
     */
    explain(riskScore, features, graphContext = {}, ruleResults = []) {
        // Determine action
    ...
    determineImpact(featureName, value, thresholds) {
    ...
    matchTypologies(features, graphContext, ruleResults) {
        const matches = [];
    ...
    generateTopReasons(factors, typologyMatches, ruleResults) {
        ...
        for (const factor of factors) {
            if ((factor.impact === ImpactLevel.CRITICAL || factor.impact === ImpactLevel.HIGH) &&
    ...
    generateSummary(riskScore, action, topReasons, typologyMatches) {
        const actionContext = {
    ...
    estimateConfidence(factors, graphContext) {
        let confidence = 0.7;
