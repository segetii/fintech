...
import { getLabelSafeguards } from '../labels/label-safeguards.js';
...
function authenticateUser(req, res, next) {
    ...
    const userRoles = (req.headers['x-user-roles'] || 'ANALYST').split(',');
    if (!userId) {
...
labelRouter.post('/:id/approve', authenticateUser, async (req, res) => {
    try {
        const user = req.user;
        const safeguards = getLabelSafeguards();
        const record = await safeguards.approveLabel(user, req.params.id, 'APPROVED', req.body.comments || '', req.ip || '0.0.0.0');
        res.json({
            success: true,
...
labelRouter.post('/:id/reject', authenticateUser, async (req, res) => {
    try {
        ...
        const record = await safeguards.approveLabel(user, req.params.id, 'REJECTED', req.body.comments, req.ip || '0.0.0.0');
        res.json({
            success: true,
            labelId: req.params.id,
            status: record.status,
