...
import { SwapModel } from "../db/models.js";
...
txRouter.post("/sendTransaction", async (req, res) => {
    ...
    // call contract
    try {
        const tx = await amttp.initiateSwapETH(seller, secretHash, Math.floor(Date.now() / 1000) + (timelockSec ?? 3600), riskLevel ?? 1, kycHash ?? ethers.ZeroHash, "0x", // oracleSig optional
        { value: amountWei });
        const rc = await tx.wait();
        // compute swapId locally (same logic as contract)
        const swapId = ethers.keccak256(ethers.AbiCoder.defaultAbiCoder().encode(["address", "address", "bytes32", "uint256", "uint8", "address", "uint256", "uint256"], [buyer, seller, secretHash, Math.floor(Date.now() / 1000) + (timelockSec ?? 3600), 0, ethers.ZeroAddress, 0, amountWei] // matches computeSwapId args
        ));
