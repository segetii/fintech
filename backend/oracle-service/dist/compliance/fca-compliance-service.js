...
class ComplianceDataStore {
     ...
     */
    markReviewed(decisionId, reviewedBy, ipAddress) {
     ...
     */
    fileSAR(decisionId, sarData, ipAddress) {
     ...
     * Submit SAR to NCA
     */
    submitSARToNCA(sarId, submittedBy, ipAddress) {
    ...
    // ── AUDIT CHAIN ─────────────────────────────────────────────────────────
    addAuditEntry(action, entityType, entityId, userId, details, ipAddress) {
        const entry = {
            id: randomUUID(),
            timestamp: new Date(),
            action,
    ...
    getAuditChain(filters) {
        ...
        if (filters?.entityId)
            chain = chain.filter(e => e.entityId === filters.entityId);
        if (filters?.userId)
            chain = chain.filter(e => e.userId === filters.userId);
        if (filters?.action)
            chain = chain.filter(e => e.action === filters.action);
        return chain;
    }
    // ── STATISTICS ──────────────────────────────────────────────────────────
    getStats() {
     ...
     */
    exportForFCAAudit(startDate, endDate) {
        const decisions = Array.from(this.decisions.values())
...
function authenticateComplianceUser(req, res, next) {
...
app.post('/compliance/ingest', async (req, res) => {
    try {
...
app.get('/compliance/pending', authenticateComplianceUser, requireRole('COMPLIANCE_OFFICER', 'MLRO'), (req, res) => {
    const pending = complianceStore.getDecisionsByStatus('PENDING');
    res.json({ count: pending.length, records: pending });
});
app.post('/compliance/review/:id', authenticateComplianceUser, requireRole('COMPLIANCE_OFFICER', 'MLRO'), (req, res) => {
    try {
        const user = req.complianceUser;
        const record = complianceStore.markReviewed(req.params.id, user.id, req.ip || '0.0.0.0');
...
// ── SAR ENDPOINTS ─────────────────────────────────────────────────────────
app.post('/compliance/sar/:decisionId', authenticateComplianceUser, requireRole('COMPLIANCE_OFFICER', 'MLRO'), (req, res) => {
    try {
        const user = req.complianceUser;
        const sar = complianceStore.fileSAR(req.params.decisionId, {
            ...req.body,
            filedBy: user.id,
        }, req.ip || '0.0.0.0');
...
app.post('/compliance/sar/:sarId/submit', authenticateComplianceUser, requireRole('MLRO'), // Only MLRO can submit to NCA
(req, res) => {
    try {
        const user = req.complianceUser;
        const sar = complianceStore.submitSARToNCA(req.params.sarId, user.id, req.ip || '0.0.0.0');
...
app.get('/compliance/sars/pending', authenticateComplianceUser, requireRole('COMPLIANCE_OFFICER', 'MLRO'), (req, res) => {
    const pending = complianceStore.getSARsPending();
    res.json({ count: pending.length, sars: pending });
});
// ── AUDIT ENDPOINTS ───────────────────────────────────────────────────────
app.get('/compliance/audit', authenticateComplianceUser, requireRole('AUDITOR', 'MLRO'), (req, res) => {
...
app.get('/compliance/audit/verify', authenticateComplianceUser, requireRole('AUDITOR', 'MLRO'), (req, res) => {
    const result = complianceStore.verifyAuditChain();
    res.json(result);
});
// ── EXPORT ENDPOINTS ──────────────────────────────────────────────────────
app.get('/compliance/export/fca', authenticateComplianceUser, requireRole('MLRO', 'AUDITOR'), (req, res) => {
    const startDate = new Date(req.query.startDate || '2024-01-01');
    const endDate = new Date(req.query.endDate || new Date().toISOString());
    const export_ = complianceStore.exportForFCAAudit(startDate, endDate);
    res.json(export_);
});
// ── STATISTICS ────────────────────────────────────────────────────────────
app.get('/compliance/stats', authenticateComplianceUser, (req, res) => {
    res.json(complianceStore.getStats());
});
// ── HEALTH CHECK ──────────────────────────────────────────────────────────
app.get('/compliance/health', (req, res) => {
    const integrity = complianceStore.verifyAuditChain();
    res.json({
...
export function startComplianceService() {
