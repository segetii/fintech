...
export interface Evidence {
  ...
  disputeId: string;
  submittedBy: string;
  evidenceType: 'TEXT' | 'FILE' | 'TRANSACTION' | 'SCREENSHOT';
  content: string;
  contentHash: string;
  ipfsUri?: string;
  submittedAt: Date;
...
export interface Dispute {
  ...
  // Parties
  claimant: string;  // Who opened the dispute
  respondent: string;
...
export class DisputeResolutionService {
  ...
  private evidencePeriodDays = 7;
  private klerosCourtId = 0; // General Court
   ...
   */
  async startVoting(disputeId: string): Promise<Dispute> {
  ...
  private calculateAvgResolutionDays(disputes: Dispute[]): number {
