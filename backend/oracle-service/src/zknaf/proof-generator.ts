 ...
 */
export async function poseidonHash(inputs: (string | bigint)[]): Promise<bigint> {
...
export interface RiskProofInput {
  riskScore: number;
  userAddress: string;
...
export class ProductionProofGenerator {
  ...
  constructor(artifactsBasePath: string, isProduction = false) {
   ...
   */
  async loadVerificationKeys(): Promise<void> {
   ...
   */
  private getArtifactPaths(circuitName: string): ProofArtifacts {
  ...
  private getVkeyPath(circuitName: string): string {
   ...
   */
  hasCircuitArtifacts(circuitName: string): boolean {
  ...
  async generateSanctionsProof(
    ...
    merkleProofData: { leftProof: string[]; rightProof: string[]; leftIndices: string[]; rightIndices: string[] }
  ): Promise<ZKProof> {
   ...
   */
  async buildSanctionsMerkleProof(
    userAddress: bigint,
    ...
    for (let i = 0; i < sorted.length; i++) {
      if (sorted[i] < userAddress) leftIdx = i;
      else break;
    }
  ...
  private async buildMerkleTree(
    leaves: string[],
    depth: number
  ): Promise<{ root: string; proofs: Array<{ elements: string[]; indices: string[] }> }> {
    const poseidon = await getPoseidon();
    const F = poseidon.F;
   ...
   */
  private generateDemoSanctionsProof(userAddress: string, sanctionsRoot: string): ZKProof {
    const timestamp = Math.floor(Date.now() / 1000);
    const userAddressHash = ethers.keccak256(
      ethers.solidityPacked(['address'], [userAddress])
    );

    // Generate deterministic demo proof data
    const proofSeed = ethers.keccak256(
      ethers.solidityPacked(['address', 'bytes32', 'uint256'], [userAddress, sanctionsRoot, timestamp])
    );
  ...
  private generateDemoRiskProof(
    ...
    const oracleCommitment = ethers.keccak256(
      ethers.solidityPacked(
        ['uint256', 'address', 'uint256', 'bytes32'],
        [riskScore, userAddress, timestamp, oracleSecret]
      )
    );
    ...
    const proofSeed = ethers.keccak256(
      ethers.solidityPacked(
        ...
        [userAddress, riskScore, minRange, maxRange]
      )
    );
  ...
  private generateDemoKYCProof(
    ...
    const kycDataHash = ethers.keccak256(
      ethers.solidityPacked(['string', 'address', 'uint256'], [kycRecordId, userAddress, completedAt])
    );

    const providerCommitment = ethers.keccak256(
      ethers.solidityPacked(['bytes32', 'bytes32'], [kycDataHash, providerSecret])
    );

    const proofSeed = ethers.keccak256(
      ethers.solidityPacked(['address', 'bytes32'], [userAddress, providerCommitment])
    );
   ...
   */
  async getCalldata(proof: ZKProof): Promise<string> {
    try {
...
let proofGeneratorInstance: ProductionProofGenerator | null = null;

export function getProofGenerator(): ProductionProofGenerator {
...
export function initProofGenerator(artifactsPath: string, isProduction = false): ProductionProofGenerator {
