...
const DEFAULT_ACTION_THRESHOLDS: ActionThresholds = {
  allow: { max: 0.40 },
...
const DEFAULT_FEATURE_CONFIGS: FeatureConfig[] = [
  // ─── Transaction Amount ─────────────────────────────────────────────────────
  {
    id: 'amountEth',
...
const DEFAULT_TYPOLOGY_CONFIGS: TypologyConfig[] = [
  {
 ...
 */
export function loadConfig(): ExplainabilityConfig {
  ...
  const config: ExplainabilityConfig = {
    ...
    logging: {
      level: (env.EXPLAIN_LOG_LEVEL as Severity) || Severity.INFO,
 ...
 */
export function getConfigHash(config: ExplainabilityConfig): string {
