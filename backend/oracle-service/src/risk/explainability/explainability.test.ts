...
describe('RiskExplainer', () => {
  ...
  describe('Typology Detection', () => {
    ...
    it('should detect dormant activation', () => {
      ...
      const dormant = result.typologyMatches.find(
        t => t.typology === TypologyType.DORMANT_ACTIVATION
      );
    ...
    it('should detect sanctions proximity typology', () => {
      ...
      const sanctions = result.typologyMatches.find(
        t => t.typology === TypologyType.SANCTIONS_PROXIMITY
      );
    ...
    it('should detect mixer interaction', () => {
      ...
      const mixer = result.typologyMatches.find(
        t => t.typology === TypologyType.MIXER_INTERACTION
      );
    ...
    it('should detect high-risk geography', () => {
      ...
      const geo = result.typologyMatches.find(
        t => t.typology === TypologyType.HIGH_RISK_GEOGRAPHY
      );
...
describe('Convenience Function', () => {
  ...
  it('should work with full input', () => {
    const result = explainRiskDecision(
      0.75,
      { amountEth: 500, dormancyDays: 180 },
      { hopsToSanctioned: 2 },
      [{ ruleType: 'LAYERING', triggered: true }],
      2500
    );
