...
export class TypologyMatcher {
  private configs: Map<TypologyType, TypologyConfig>;
  
  constructor(configs?: TypologyConfig[]) {
    const typologyConfigs = configs ?? loadConfig().typologies;
    this.configs = new Map(
      typologyConfigs.filter(t => t.enabled).map(t => [t.type, t])
    );
  }
  ...
  private matchFromRules(ruleResults: RuleResult[]): TypologyMatch[] {
    const matches: TypologyMatch[] = [];
  ...
  private ruleToTypology(rule: RuleResult): TypologyMatch | null {
    ...
    switch (rule.ruleType.toUpperCase()) {
      ...
      case 'SMURFING':
        return this.createMatch(
          ...
          rule.confidence ?? 0.8,
          'Multiple transactions structured just below reporting thresholds to avoid detection',
          [
      ...
      case 'LAYERING':
        return this.createMatch(
          TypologyType.LAYERING,
          rule.confidence ?? 0.85,
          'Complex transaction chain detected, potentially to obscure the origin of funds',
          [
            `Chain length: ${evidence.chainLength ?? '?'} hops`,
            `Value preserved: ${((evidence.valuePreserved as number ?? 0) * 100).toFixed(0)}%`,
      ...
      case 'ROUND_TRIP':
        return this.createMatch(
          TypologyType.ROUND_TRIP,
          rule.confidence ?? 0.75,
          'Funds returned to original sender through intermediary addresses',
          [
      ...
      case 'DISTRIBUTION':
        return this.createMatch(
          TypologyType.FAN_OUT,
          rule.confidence ?? 0.7,
          'Funds distributed to many recipients in rapid succession',
          [
      ...
      case 'AGGREGATION':
        return this.createMatch(
          TypologyType.FAN_IN,
          rule.confidence ?? 0.7,
          'Funds aggregated from many sources in rapid succession',
          [
  ...
  private matchFromFeatures(
    ...
    if (features.dormancyDays && features.dormancyDays >= 90) {
      ...
      matches.push(this.createMatch(
        TypologyType.DORMANT_ACTIVATION,
        confidence,
        'Account reactivated after extended dormancy period with significant activity',
        [
    ...
    if (features.fatfCountryRisk === 'blacklist' || features.fatfCountryRisk === 'greylist') {
      ...
      matches.push(this.createMatch(
        TypologyType.HIGH_RISK_GEOGRAPHY,
        confidence,
        `Transaction involves FATF ${features.fatfCountryRisk}ed jurisdiction`,
        [
    ...
    if (features.pepMatch) {
      matches.push(this.createMatch(
        TypologyType.PEP_INVOLVEMENT,
        0.85,
        'Transaction involves Politically Exposed Person',
        [
    ...
    if (features.uniqueRecipients24h && features.uniqueRecipients24h >= 15) {
      matches.push(this.createMatch(
        TypologyType.FAN_OUT,
        Math.min(0.9, 0.5 + features.uniqueRecipients24h / 50),
        'Funds distributed to many recipients, potential distribution pattern',
        [
  ...
  private matchFromGraph(graphContext: GraphContext): TypologyMatch[] {
    const matches: TypologyMatch[] = [];
    ...
    if (graphContext.hopsToSanctioned !== undefined && graphContext.hopsToSanctioned <= 3) {
      ...
      matches.push(this.createMatch(
        TypologyType.SANCTIONS_PROXIMITY,
        confidence,
        hops === 0 
          ? 'DIRECT SANCTIONS MATCH - Address on consolidated sanctions list'
          : `Transaction chain ${hops} hop(s) from sanctioned entity`,
        hops === 0
          ? [
    ...
    if (graphContext.mixerInteraction || 
      ...
      matches.push(this.createMatch(
        TypologyType.MIXER_INTERACTION,
        confidence,
 ...
 */
export function getTypologyDisplayName(typology: TypologyType): string {
  const names: Record<TypologyType, string> = {
