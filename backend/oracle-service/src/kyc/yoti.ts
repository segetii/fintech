...
interface YotiSessionConfig {
  userId: string;
  verificationType?: "IDENTITY" | "DOCUMENT" | "FACE_MATCH";
  redirectUrl?: string;
}

interface YotiSession {
  sessionId: string;
 ...
 */
export async function yotiCreateSession(userId: string, config?: Partial<YotiSessionConfig>): Promise<YotiSession> {
  validateYotiConfig();
 ...
 */
export async function yotiGetSession(sessionId: string): Promise<YotiSessionResult> {
  validateYotiConfig();
 ...
 */
export async function yotiDeleteSession(sessionId: string): Promise<void> {
  validateYotiConfig();
...
function validateYotiConfig(): void {
...
function signRequest(method: string, endpoint: string, body?: string): { signature: string; nonce: string; timestamp: string } {
...
function buildSessionSpec(userId: string, config?: Partial<YotiSessionConfig>) {
  return {
...
function mapYotiStatus(state: string, checks: any[]): "approved" | "pending" | "rejected" | "expired" {
  if (state === "COMPLETED") {
    // Check if all checks passed
    const allPassed = checks.every(
      (c: any) => c.state === "DONE" && c.report?.recommendation?.value === "APPROVE"
    );
    const anyRejected = checks.some(
      (c: any) => c.report?.recommendation?.value === "REJECT"
    );
...
function extractAttributes(data: any): Record<string, any> {
