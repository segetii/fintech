...
type CreateApplicantParams = { externalUserId: string; level?: string };

export async function sumsubCreateApplicant(p: CreateApplicantParams) {
  ...
  if (process.env.SUMSUB_SANDBOX) headers["X-REQUEST-ID"] = `sandbox-${p.externalUserId}-${Date.now()}`;

  const { data } = await axios.post(url, body, { headers });
  return data; // contains applicantId, etc.
}

export async function sumsubGetApplicant(applicantId: string) {
  const base = process.env.SUMSUB_BASE_URL!;
  const appToken = process.env.SUMSUB_APP_TOKEN!;
  const secret = process.env.SUMSUB_SECRET_KEY!;
...
function sign(method: string, path: string, ts: number, body: string, secret: string) {
