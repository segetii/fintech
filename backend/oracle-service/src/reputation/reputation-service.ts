...
export interface ReputationEvent {
  id: string;
  address: string;
  eventType: 'TRANSACTION' | 'DISPUTE' | 'VERIFICATION' | 'STAKE' | 'SLASH';
...
export class ReputationService {
   ...
   */
  getOrCreateReputation(address: string): ReputationScore {
  ...
  recordSuccessfulTransaction(params: {
    ...
    score.reliabilityScore = Math.min(100, 
  ...
  recordFailedTransaction(params: {
    ...
    // Impact reliability
    score.reliabilityScore = Math.max(0,
      (score.successfulTransactions / score.totalTransactions) * 100
    );
   ...
   */
  getTierBenefits(address: string): typeof TIER_BENEFITS[keyof typeof TIER_BENEFITS] & { tier: string } {
   ...
   */
  getEvents(address: string, limit = 50): ReputationEvent[] {
    return this.events
   ...
   * Get leaderboard
   */
  getLeaderboard(limit = 100): ReputationScore[] {
  ...
  private recalculateOverall(score: ReputationScore): void {
    // Weighted average
    ...
    // Update account age
    if (score.firstTransactionAt) {
      score.accountAgedays = Math.floor(
        (Date.now() - score.firstTransactionAt.getTime()) / (24 * 60 * 60 * 1000)
      );
    }
  ...
  private logEvent(event: Omit<ReputationEvent, 'id' | 'timestamp'>): void {
    this.events.push({
      ...event,
      id: randomUUID(),
      timestamp: new Date(),
