...
export class OngoingMonitoringService extends EventEmitter {
  private entities: Map<string, MonitoredEntity> = new Map();
  private alerts: Map<string, MonitoringAlert> = new Map();
  private runs: MonitoringRun[] = [];
  
  private defaultRescreenDays = 30;
   ...
   */
  updateEntity(id: string, updates: Partial<MonitoredEntity>): MonitoredEntity {
    ...
    for (const key of allowed) {
      if (key in updates) {
        (entity as any)[key] = (updates as any)[key];
      }
    }
   ...
   * Get entity by ID
   */
  getEntity(id: string): MonitoredEntity | undefined {
   ...
   */
  listEntities(activeOnly = true): MonitoredEntity[] {
   ...
   */
  getEntitiesDueForScreening(): MonitoredEntity[] {
    const now = new Date();
   ...
   */
  async screenEntity(entityId: string): Promise<MonitoringAlert[]> {
   ...
   * Run batch screening
   */
  async runBatchScreening(type?: MonitoringType): Promise<MonitoringRun> {
   ...
   * Acknowledge alert
   */
  acknowledgeAlert(alertId: string, acknowledgedBy: string): MonitoringAlert {
   ...
   */
  getOpenAlertsCount(): Record<AlertSeverity, number> {
   ...
   */
  startAutomatedMonitoring(intervalMinutes = 60): void {
  ...
  private calculateNewRiskScore(entity: MonitoredEntity, newAlerts: MonitoringAlert[]): number {
  ...
  private calculateTrend(entity: MonitoredEntity): MonitoredEntity['riskTrend'] {
...
monitoringRouter.get('/alerts', (req, res) => {
  const alerts = monitoringService.listAlerts({
    ...
    type: req.query.type as MonitoringType,
  });
...
monitoringRouter.post('/alert/:id/acknowledge', (req, res) => {
  try {
    const alert = monitoringService.acknowledgeAlert(
      req.params.id,
      req.body.acknowledgedBy || 'system'
    );
