...
export type ReviewOutcome = 'APPROVE' | 'REJECT' | 'ESCALATE' | 'REQUEST_MORE';
...
export interface EDDCase {
  ...
  subjectId: string;  // Customer/entity ID
  subjectName: string;
  ...
  triggerEvent?: string;  // Transaction ID, alert ID, etc.
  riskScore: number;
...
export interface ReviewAction {
  id: string;
  action: 'CREATE' | 'ASSIGN' | 'REQUEST_DOCS' | 'SUBMIT_DOC' | 'REVIEW' | 'APPROVE' | 'REJECT' | 'ESCALATE' | 'COMMENT';
  performedBy: string;
  performedAt: Date;
  notes?: string;
...
export class EDDService {
  private cases: Map<string, EDDCase> = new Map();
  private defaultDueDays = 14;
  
  // Document requirements by trigger
  private documentRequirements: Record<string, DocumentType[]> = {
   ...
   */
  requestDocuments(caseId: string, requestedBy: string, additionalDocs?: DocumentType[]): EDDCase {
    const eddCase = this.cases.get(caseId);
    if (!eddCase) throw new Error('Case not found');
   ...
   */
  assignCase(caseId: string, assignedTo: string, assignedBy: string): EDDCase {
    const eddCase = this.cases.get(caseId);
    if (!eddCase) throw new Error('Case not found');
   ...
   * Get case by ID
   */
  getCase(id: string): EDDCase | undefined {
   ...
   * Get cases by status
   */
  getCasesByStatus(status: EDDStatus): EDDCase[] {
   ...
   */
  getCasesByAssignee(assignee: string): EDDCase[] {
  ...
  private calculatePriority(riskScore: number): EDDCase['priority'] {
  ...
  private calculateAvgResolution(cases: EDDCase[]): number {
...
eddRouter.post('/case/:id/request', (req, res) => {
  try {
    const eddCase = eddService.requestDocuments(
      req.params.id,
      req.body.requestedBy || 'system',
      req.body.additionalDocs
    );
    res.json({
      success: true,
      status: eddCase.status,
...
eddRouter.post('/case/:id/assign', (req, res) => {
  try {
    const eddCase = eddService.assignCase(
      req.params.id,
      req.body.assignedTo,
...
eddRouter.get('/cases', (req, res) => {
  ...
  if (req.query.status) {
    cases = eddService.getCasesByStatus(req.query.status as EDDStatus);
  } else if (req.query.assignee) {
    cases = eddService.getCasesByAssignee(req.query.assignee as string);
  } else if (req.query.overdue === 'true') {
    cases = eddService.getOverdueCases();
  } else {
    cases = Array.from((eddService as any).cases.values());
  }
