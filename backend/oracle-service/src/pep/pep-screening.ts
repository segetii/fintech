...
export type DataProvider = 'DOW_JONES' | 'REFINITIV' | 'OPEN_SANCTIONS' | 'UN_SC' | 'OFAC' | 'EU_CONSOLIDATED';
...
export type MatchConfidence = 'EXACT' | 'STRONG' | 'POTENTIAL' | 'WEAK';
...
export interface ScreeningResult {
  ...
  highestConfidence: MatchConfidence | null;
  isPEP: boolean;
  isSanctioned: boolean;
  
  // Processing
  providersQueried: DataProvider[];
...
const MOCK_PEP_ENTRIES: PEPEntry[] = [
  {
    id: 'pep-001',
    provider: 'DOW_JONES',
    name: 'Vladimir Putin',
...
export class PEPScreeningService {
  private cache: Map<string, ScreeningResult> = new Map();
  private cacheTimeoutMs = 24 * 60 * 60 * 1000; // 24 hours
  private providers: DataProvider[] = ['DOW_JONES', 'REFINITIV', 'OPEN_SANCTIONS'];
  
  // Simulated API credentials (would be in env vars)
  private apiKeys: Record<DataProvider, string | null> = {
   ...
   * Get provider status
   */
  getProviderStatus(): Record<DataProvider, { configured: boolean; healthy: boolean }> {
    const status: Record<string, { configured: boolean; healthy: boolean }> = {};
    
    for (const provider of Object.keys(this.apiKeys) as DataProvider[]) {
    ...
    return status as Record<DataProvider, { configured: boolean; healthy: boolean }>;
  }
  ...
  private getCacheKey(name: string, dob?: string): string {
  ...
  private async queryProvider(
    provider: DataProvider,
    params: { name: string; dateOfBirth?: string; nationality?: string }
    ...
    for (const entry of MOCK_PEP_ENTRIES) {
      ...
      const { score, reasons, confidence } = this.calculateMatch(
        queryNameLower,
        entry,
        params.dateOfBirth,
        params.nationality
  ...
  private calculateMatch(
    ...
    entry: PEPEntry,
    queryDob?: string,
    queryNationality?: string
  ): { score: number; reasons: string[]; confidence: MatchConfidence } {
    let score = 0;
