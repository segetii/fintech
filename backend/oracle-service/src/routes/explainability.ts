...
import { 
  ...
  RuleResult,
  RiskAction
} from '../risk/explainability/index.js';
...
let service: ExplainabilityService | null = null;

function getService(): ExplainabilityService {
  if (!service) {
...
explainabilityRouter.get('/', (_req: Request, res: Response) => {
  res.json({
 ...
 */
explainabilityRouter.post('/explain', async (req: Request, res: Response) => {
 ...
 */
explainabilityRouter.post('/explain/transaction', async (req: Request, res: Response) => {
  const startTime = Date.now();
  const requestId = req.headers['x-request-id'] as string;
  ...
  try {
    ...
    res.json({
      ...
      auditData: {
        ...
        features: Object.keys(features).filter(k => features[k as keyof RiskFeatures] !== undefined)
      }
    });
 ...
 */
explainabilityRouter.get('/health', async (_req: Request, res: Response) => {
  try {
    const svc = getService();
 ...
 */
explainabilityRouter.get('/metrics', async (_req: Request, res: Response) => {
  try {
    const svc = getService();
 ...
 */
explainabilityRouter.get('/config', (_req: Request, res: Response) => {
  res.json({
    version: '1.0.0',
    actionThresholds: {
      ALLOW: '< 0.30',
 ...
 */
explainabilityRouter.get('/typologies', (_req: Request, res: Response) => {
  res.json({
    typologies: [
      {
        id: 'LAYERING',
 ...
 */
explainabilityRouter.get('/cache/stats', async (_req: Request, res: Response) => {
  try {
    const svc = getService();
    const health = svc.getHealth();
    res.json({
 ...
 */
explainabilityRouter.post('/cache/clear', async (req: Request, res: Response) => {
  try {
