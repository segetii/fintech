...
Numerically verifies the central claim of Theorem C:
    ...
    ?E_BS(X) and ???(X) point in the same direction.
...
at each step as the system evolves.  Theorem C predicts cos ? > 0.7
...
- E_BS(X) = ?? ?(??(X)) + ??<? ?(?X??X??)   [blind-spot energy]
       ?(X) = (alpha/2)???x????? + pairwise         [GravityEngine potential]
...
PSI_SCALE  = 1.0        # ?(?) = PSI_SCALE ? ??  (convex, increasing)
...
class BSDTEnergyOperator:
    ...
    Computes E_BS(X) = ?? ?(??(X)) + pairwise(X)

    The deviation operator ??(X) is the squared Mahalanobis distance
    ...
    ?(?) = ?  (identity: convex, increasing, 1-Lipschitz)
    """
    ...
    def gradient(self, X: np.ndarray) -> np.ndarray:
        ...
        Deviation term: ?/?x? [?(??)] = ?'(??) ? 2????(x? ? ??)
        For ?(?) = ?: ?'(?) = 1, so:
            ?E_dev/?x? = 2????(x? ? ??)

        Pairwise term: same as GravityEngine ??_pair, computed via
        ...
        # pairwise terms are *identical* between E_BS and ? - they
    ...
    def gradient_fast(self, X: np.ndarray) -> np.ndarray:
        ...
        The pairwise force from gravity.py is -??_pair, so ?E_BS_pair = +??_pair
        ...
        if GRAVITY_AVAILABLE:
            # Get pairwise force from gravity.py (= -??_pair)
...
# ?????????????????????????????????????????????
# GravityEngine force: ???(X)
# ?????????????????????????????????????????????

def gravity_force(X: np.ndarray, mu: np.ndarray) -> np.ndarray:
    """Total GravityEngine force = ???(X).  Shape (N, d)."""
    if GRAVITY_AVAILABLE:
...
def run_experiment():
    ...
    print(f"      ?? = {op.mu0_.round(3)}")
    print(f"      ???? diagonal = {np.diag(op.Sigma0_inv_).round(3)}")
    ...
    print(f"\n[sim] Running {N_STEPS} steps, N={N_AGENTS}, d={D_DIM}, ?={ETA}")
    ...
    for t in range(N_STEPS):
        ...
        # Evolve: gradient flow step X <- X + ? ? F_gravity
        dX = ETA * F_gravity
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"Mean cosine ?:          {mean_cos:.4f}\n")
        f.write(f"Median cosine ?:        {median_cos:.4f}\n")
        f.write(f"Min cosine ?:           {min_cos:.4f}\n")
        ...
        f.write(f"  alpha={ALPHA}, gamma={GAMMA}, ?={SIGMA}, lambda={LAMBDA_REP}\n")
        f.write(f"  GravityEngine available: {GRAVITY_AVAILABLE}\n\n")
        f.write(f"Interpretation:\n  {summary['interpretation']}\n\n")
        f.write("What cos ? measures:\n")
        f.write("  cos ? = ??E_BS(X), ???(X)? / (??E_BS? ? ?????)\n")
        f.write("  cos ? = 1.0  ->  perfect alignment (same direction)\n")
        f.write("  cos ? = 0.0  ->  orthogonal (no overlap)\n")
        f.write("  cos ? = ?1.0 ->  anti-aligned (opposite)\n")
        f.write("\nTheorem C predicts cos ? ? 0.7 on average.\n")
        f.write("If this holds, ?E_BS is a valid substitute for ???\n")
    ...
    fig.suptitle("Gradient Alignment Verification - Theorem C\n"
                 ...
                 f"(mean cos ? = {mean_cos:.3f})", fontsize=13)

    steps = np.arange(len(cos_mean_history))

    # Panel 1: mean per-agent cosine
    ax = axes[0, 0]
    ax.plot(steps, cos_mean_history, lw=1.5, color="steelblue", label="mean per-agent cos ?")
    ...
    ax.set_xlabel("Step"); ax.set_ylabel("cos ? (mean per-agent)")
    ax.set_title("Per-agent cosine alignment"); ax.legend(fontsize=8); ax.grid(alpha=0.3)

    # Panel 2: global cosine
    ax = axes[0, 1]
    ax.plot(steps, cos_global_history, lw=1.5, color="darkorange", label="global cos ?")
    ax.axhline(THRESHOLD, color="red", ls="--", lw=1)
    ax.axhline(0, color="gray", ls=":", lw=0.8)
    ax.set_xlabel("Step"); ax.set_ylabel("cos ? (global)")
    ...
    # Panel 4: histogram of cos ? values
    ax = axes[1, 1]
    ...
    ax.set_xlabel("cos ?"); ax.set_ylabel("Count")
    ax.set_title("Distribution of per-step cos ?"); ax.legend(fontsize=8); ax.grid(alpha=0.3)
