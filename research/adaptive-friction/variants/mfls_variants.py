...
class MFLSBaseline:
    """MFLS = ||?E_BS||_F = ||2 ???? (X - ??)||_F  (current pipeline)."""
...
class MFLSFullBSDT:
    """
    MFLS = ?_k ?_k(X)  with uniform weights.
...
class MFLSQuadSurf:
    ...
    Score = beta? + ?_k beta_k?c_k + ?_{k?j} beta_{kj}?c_k?c_j
    """
    ...
    def fit(self, channels_train: np.ndarray, y: np.ndarray):
        ...
        # Ridge regression: beta = (?? ? + alphaI)?? ?? y
        n_feat = Phi.shape[1]
        I = np.eye(n_feat)
...
class MFLSSignedLR:
    ...
    P(crisis | c?,...,c?) = ?(beta? + ?_k beta_k?c_k)
    """
