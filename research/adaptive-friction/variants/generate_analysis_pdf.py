...
def build_pdf():
    ...
    meta_data = [
        ...
        ["Network", f"Ledoit-Wolf Oracle shrinkage,  ρ* = {v2['lw_shrinkage_rho_star']}"],
    ...
    story.append(Paragraph("1. The Alignment Theorem Fails on Real Data (cos θ = −0.34)", s_h2))
    story.append(Paragraph(
        "The paper's central mathematical claim (Theorem C) predicts that the blind-spot energy "
        "gradient ∇E<sub>BS</sub> should be <b>aligned</b> with the restoring force −∇Φ, meaning "
        "the detector 'points toward' equilibrium. On real FDIC data:", s_body))

    align_data = [
        ["Measurement", "cos θ", "Verdict"],
    ...
    story.append(Paragraph(
        ...
        "anomaly detection are perpendicular phenomena</b>. The radial+pairwise potential Φ pulls "
        ...
        "Φ cares about <i>where agents are relative to each other right now</i>, while E<sub>BS</sub> "
    ...
    story.append(Paragraph("γ* = α / λ_max(D²Φ_pair)", s_eq))
    story.append(Paragraph(
        "The engine computes the spectral radius λ<sub>max</sub> of the pairwise Hessian at each "
        "quarter via power iteration, then the adaptive coupling γ*.", s_body))

    crit_data = [
        ["Quantity", "Value"],
        ["Fraction of quarters with λ_max > α", f"{v2['frac_above_cman']:.1%}"],
        ["Mean γ*", f"{v2['gamma_star_mean']:,.1f}"],
        ["Network λ_max(W)  (Ledoit-Wolf)", f"{v2['lambda_max_W']:.4f}"],
        ["Ledoit-Wolf shrinkage ρ*", f"{v2['lw_shrinkage_rho_star']:.4f}"],
    ]
    ...
    # ── Discovery 6: Network ──
    story.append(Paragraph("6. Ledoit-Wolf Network: ρ* = 0.046, λ_max = 2.98", s_h2))
    story.append(Paragraph(
        ...
        "(ii) the resulting λ<sub>max</sub>(W) = 2.98 is the empirical spectral radius of the real "
        ...
        "γ &gt; α / λ<sub>max</sub>(W) = 0.10 / 2.98 = 0.034.", s_body))
    ...
    variants_desc = [
        ("<b>1. Baseline</b> — Mahalanobis gradient norm: "
         "MFLS = ‖2Σ₀⁻¹(X − μ₀)‖<sub>F</sub>. The current pipeline's scoring function. "
         "Unsupervised, no crisis labels needed."),
        ("<b>2. Full BSDT</b> — Uniform-weighted 4-channel sum: "
         "MFLS = Σ<sub>k</sub> δ<sub>k</sub>(X) with k ∈ {C, G, A, T}. "
        ...
        ("<b>3. QuadSurf</b> — Degree-2 polynomial ridge regression on BSDT channels. "
         ...
         "Ridge regularisation (α = 1.0) prevents overfitting on sparse crisis labels."),
        ("<b>4. Signed LR</b> — Logistic regression on BSDT channels. "
         "P(crisis | c₁,...,c₄) = σ(β₀ + Σ<sub>k</sub> β<sub>k</sub>·c<sub>k</sub>). "
    ...
    lr_data = [
        ["Channel", "Weight β", "Interpretation"],
        ["Bias (β₀)", "−4.16", "Strong prior: crises are rare"],
        ["δ_C (Camouflage)", "+1.52", "Banks hiding risk = strongest signal"],
        ["δ_A (Activity)", "+1.23", "Rapid state changes = second-strongest"],
        ["δ_G (Feature Gap)", "+0.38", "Low-variance displacement = weak positive"],
        ["δ_T (Temporal)", "−1.38", "Low novelty = crisis precursor"],
    ]
    story.append(_make_table(lr_data))
    story.append(Spacer(1, 2*mm))
    story.append(Paragraph(
        "The <b>negative δ<sub>T</sub> weight is the most striking finding</b>. It says that before "
    ...
    story.append(Paragraph(
        "<b>(i) Uniform weights dilute the dominant signal.</b> δ<sub>C</sub> has mean = 137 while "
        "δ<sub>A</sub> has mean = 0.35. After normalisation, each gets equal weight, but δ<sub>A</sub> "
        "is sparse and noisy. Uniform weighting injects noise.", s_body))
    story.append(Paragraph(
        "<b>(ii) The channels are not independent.</b> corr(δ<sub>C</sub>, SRISK) = 0.126 while "
        "corr(δ<sub>T</sub>, SRISK) = 0.163 — partially overlapping. Adding partially redundant "
    ...
    story.append(Paragraph(
        ...
        "uses MFLS(X) = Σ w<sub>k</sub> · S<sub>k</sub>(X) where weights are <i>learned</i>. "
    ...
    discoveries = [
        ("<b>1. Anti-alignment (cos θ = −0.34):</b> Equilibrium-seeking and anomaly detection "
    ...
    claims = [
        ...
        ("<b>Claim 2 — Granger non-significance is a structural feature, not a bug:</b> Five "
         "independent scoring functions using different functional families all fail at α = 0.10. "
    ...
    story.append(Paragraph(
        ...
        "on temporal novelty</b> (β<sub>δ_T</sub> = −1.38). This means crises are preceded not by "
