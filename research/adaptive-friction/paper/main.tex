...
the MFLS signal achieves AUROC = 0.81 at the bank level and 0.78 on the
cross-border panel (Baseline variant, using exclusively public data), fires
its GFC alarm 6~quarters before Lehman at every scale, and passes three
pre-registered robustness checks (alternate normal period, rolling refit,
...
\begin{table}[htbp]
\centering
\caption{Cross-scale synthesis: Baseline MFLS performance at three levels of aggregation.
All results use real data only.}
\label{tab:synthesis}
\begin{tabular}{lccccc}
\toprule
Configuration & $N$ & $T$ & AUROC & GFC Lead & Granger \\
\midrule
Institution-type (FDIC aggregated) & 7  & 140 & $\geq 0.70$ & 6Q & FAIL \\
Bank-level (FDIC individual)       & 30 & 140 & 0.805       & 6Q & FAIL \\
G-SIB cross-border (real data)     & 20 &  76 & 0.781       & 6Q & PASS \\
\bottomrule
\end{tabular}
\end{table}
...
a data artefact;
(ii)~AUROC improves from 0.70 (7 institution types) to 0.81 (30 banks),
and the cross-border panel achieves 0.78 despite coarser country-level
data---confirming that the signal is robust across heterogeneous data
sources;
...
exclusively real data, produces eight findings:
...
\begin{enumerate}
  ...
  \item \textbf{Precision-recall frontier.}  On the $N=7$ panel, variants
    trade off monotonically between hit rate and selectivity, tracing a
    Pareto curve that separates ``distance from equilibrium'' from
  ...
  \item \textbf{Cross-border robustness.}  The Baseline AUROC on the
    $N=20$ G-SIB panel (0.781) is comparable to the domestic bank-level
    result (0.805), despite using coarser country-level data for non-U.S.
    institutions---confirming that the BSDT signal generalises across
