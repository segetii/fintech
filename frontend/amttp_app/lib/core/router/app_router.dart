...
FintechSection fintechSectionForRoute(String route) {
  return _sectionFromPath(route.split('?').first.split('#').first) ==
          PremiumSection.home
      ? FintechSection.home
      : FintechSection.values[
          _sectionFromPath(route.split('?').first.split('#').first).index];
}
...
final routerProvider = Provider<GoRouter>((ref) {
  ...
  return GoRouter(
    ...
    redirect: (context, state) {
      ...
      // ═══════════════════════════════════════════════════════════════════════
      // DEBUG PREVIEW MODE - Removed for production security
      ...
      if (isAuthenticated) {
        ...
        if (!canRoleAccessRoute(currentRole, path)) {
          debugPrint(
              '[RouteGuard] Access denied to $path for role ${currentRole.displayName}');
      ...
      ShellRoute(
        ...
        routes: [
          ...
          GoRoute(
            ...
            builder: (context, state) =>
                const WarRoomNextJSPage(nextPath: '/war-room/alerts'),
          ),
          GoRoute(
            path: '/war-room/transactions',
            name: 'war-room-transactions',
            builder: (context, state) =>
                const WarRoomNextJSPage(nextPath: '/war-room/transactions'),
          ),
          GoRoute(
            path: '/war-room/cross-chain',
            name: 'war-room-cross-chain',
            builder: (context, state) =>
                const WarRoomNextJSPage(nextPath: '/war-room/cross-chain'),
          ),
          GoRoute(
            path: '/war-room/detection/graph',
            name: 'war-room-detection-graph',
            builder: (context, state) =>
                const WarRoomNextJSPage(nextPath: '/war-room/detection/graph'),
          ),
          GoRoute(
            path: '/war-room/detection/models',
            name: 'war-room-detection-models',
            builder: (context, state) =>
                const WarRoomNextJSPage(nextPath: '/war-room/detection/models'),
          ),
          GoRoute(
            path: '/war-room/detection/risk',
            name: 'war-room-detection-risk',
            builder: (context, state) =>
                const WarRoomNextJSPage(nextPath: '/war-room/detection/risk'),
          ),
          GoRoute(
            path: '/war-room/approvals',
            name: 'war-room-approvals',
            builder: (context, state) =>
                const WarRoomNextJSPage(nextPath: '/war-room/approvals'),
          ),
