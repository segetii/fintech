...
% ORCID icon support
\usepackage{tikz}
\newcommand{\orcidicon}{\href{https://orcid.org/0009-0004-2309-6436}{\raisebox{-0.1em}{\begin{tikzpicture}[baseline=-0.2em]
...
Beyond AUC, we introduce \textbf{top-$k$ coverage}---the fraction of true anomalies surfaced within the highest-ranked predictions---revealing that methods with comparable AUC exhibit dramatically different detection profiles. A Spearman correlation audit across 14 spectrum operators finds only one redundant pair ($\rho > 0.85$), empirically confirming that each mathematical perspective captures genuinely independent signal. Among four projection strategies evaluated on coverage, \textbf{Hybrid auto-selection} achieves 91\% mean coverage across five datasets, while \textbf{Quadratic Surface calibration} yields the best worst-case coverage (69\% minimum across datasets).
...
\begin{IEEEkeywords}
Anomaly detection, multi-spectrum representation, tensor decomposition, dimension magnification, quadratic discriminant analysis, domain adaptation, unsupervised detection, cross-domain generalisation, operator correlation, coverage analysis
\end{IEEEkeywords}
...
has full column rank, i.e., $\text{rank}(J(\boldsymbol{\mu})) = m$.

\textit{Proof.} The null space of $J(\boldsymbol{\mu})$ equals $\bigcap_{k=1}^K \ker(D\Phi_k(\boldsymbol{\mu}))$, since $J(\boldsymbol{\mu})\mathbf{v} = \mathbf{0}$ if and only if $D\Phi_k(\boldsymbol{\mu})\mathbf{v} = \mathbf{0}$ for every $k$. Full column rank means the null space is $\{\mathbf{0}\}$, which is exactly the separating condition~\eqref{eq:separating}. $\square$

\vspace{0.5em}

\textbf{Theorem 1 (Universal Deviation --- Local Form).} \textit{Let $\{\Phi_k\}_{k=1}^K$ be a deviation-separating family at the normal centroid $\boldsymbol{\mu}$, with each $\Phi_k$ continuously differentiable. Then there exists an open neighbourhood $U \ni \boldsymbol{\mu}$ such that:}

\begin{equation}
\forall\, \mathbf{x} \in U \setminus \{\boldsymbol{\mu}\},\quad \exists\, k \in \{1,\ldots,K\} \quad\text{s.t.}\quad \Phi_k(\mathbf{x}) \neq \Phi_k(\boldsymbol{\mu})
\label{eq:theorem_local}
\end{equation}

\textit{In words: no point near the centroid can match normal data in all operator domains simultaneously, unless it is the centroid itself.}

\textit{Proof.} Define the joint map $\Phi: \R^m \to \R^D$ by $\Phi(\mathbf{x}) = (\Phi_1(\mathbf{x}), \ldots, \Phi_K(\mathbf{x}))$. Its Jacobian at $\boldsymbol{\mu}$ is the stacked matrix $J(\boldsymbol{\mu})$. By the separating condition (Lemma~1), $\text{rank}(J(\boldsymbol{\mu})) = m$. By the inverse function theorem, $\Phi$ is locally injective at $\boldsymbol{\mu}$: there exists an open neighbourhood $U$ of $\boldsymbol{\mu}$ such that $\Phi|_U$ is one-to-one. Therefore, for any $\mathbf{x} \in U$ with $\mathbf{x} \neq \boldsymbol{\mu}$, we have $\Phi(\mathbf{x}) \neq \Phi(\boldsymbol{\mu})$, which means $\Phi_k(\mathbf{x}) \neq \Phi_k(\boldsymbol{\mu})$ for at least one $k$. $\square$

\vspace{0.5em}

\textbf{Corollary 1 (Quantitative deviation bound).} Under the conditions of Theorem~1, let $\sigma_{\min} > 0$ denote the smallest singular value of $J(\boldsymbol{\mu})$. Then for $\mathbf{x}$ sufficiently close to $\boldsymbol{\mu}$:
...
\textit{Proof.} By first-order Taylor expansion, $\Phi(\mathbf{x}) - \Phi(\boldsymbol{\mu}) = J(\boldsymbol{\mu})(\mathbf{x} - \boldsymbol{\mu}) + O(\norm{\mathbf{x}-\boldsymbol{\mu}}^2)$. Taking norms: $\norm{\Phi(\mathbf{x}) - \Phi(\boldsymbol{\mu})} \geq \sigma_{\min} \norm{\mathbf{x} - \boldsymbol{\mu}} - O(\norm{\mathbf{x}-\boldsymbol{\mu}}^2)$. Since $\norm{\Phi(\mathbf{x}) - \Phi(\boldsymbol{\mu})}^2 = \sum_k \norm{\Phi_k(\mathbf{x}) - \Phi_k(\boldsymbol{\mu})}^2$, the largest term satisfies $\max_k \norm{\Phi_k(\mathbf{x}) - \Phi_k(\boldsymbol{\mu})} \geq \norm{\Phi(\mathbf{x}) - \Phi(\boldsymbol{\mu})}/\sqrt{K}$, giving the result. $\square$

\textbf{Remark (Local vs.\ global).} Theorem~1 is a local result: it guarantees anomaly visibility in a neighbourhood of $\boldsymbol{\mu}$. Extension to the full anomaly set $\mathcal{A}$ requires additional structure, which we now provide.

\vspace{0.5em}

\textbf{Theorem 2 (Universal Deviation --- Global Form).} \textit{Let $\mathcal{K} \subset \R^m$ be a compact set containing both the normal support $\text{supp}(P_{\mathcal{N}})$ and the anomaly set $\mathcal{A}$, and let $\{\Phi_k\}_{k=1}^K$ be $C^1$ operators that are deviation-separating at every $\mathbf{x} \in \mathcal{K}$ (i.e., $\text{rank}(J(\mathbf{x})) = m$ for all $\mathbf{x} \in \mathcal{K}$). Then the joint map $\Phi$ is a local diffeomorphism on $\mathcal{K}$, and:}

\begin{equation}
\inf_{\mathbf{x} \in \mathcal{A}} \max_{k} \norm{\Phi_k(\mathbf{x}) - \Phi_k(\boldsymbol{\mu})} \geq \frac{\underline{\sigma}}{\sqrt{K}} \cdot \inf_{\mathbf{x} \in \mathcal{A}} \norm{\mathbf{x} - \boldsymbol{\mu}} - C \cdot \sup_{\mathbf{x} \in \mathcal{A}} \norm{\mathbf{x} - \boldsymbol{\mu}}^2
\label{eq:global}
\end{equation}

\textit{where $\underline{\sigma} := \min_{\mathbf{x} \in \mathcal{K}} \sigma_{\min}(J(\mathbf{x})) > 0$ and $C$ depends on the Lipschitz constant of $J$.}

\textit{Proof.} The map $\mathbf{x} \mapsto \sigma_{\min}(J(\mathbf{x}))$ is continuous (since each $\Phi_k$ is $C^1$, the entries of $J$ are continuous, and singular values depend continuously on the matrix). The set $\mathcal{K}$ is compact. By the extreme value theorem, $\sigma_{\min}(J(\mathbf{x}))$ attains its minimum on $\mathcal{K}$. By hypothesis, $\text{rank}(J(\mathbf{x})) = m$ everywhere on $\mathcal{K}$, so $\underline{\sigma} := \min_{\mathbf{x} \in \mathcal{K}} \sigma_{\min}(J(\mathbf{x})) > 0$.

For any $\mathbf{x} \in \mathcal{A} \subset \mathcal{K}$, Taylor expansion gives:
$$\Phi(\mathbf{x}) - \Phi(\boldsymbol{\mu}) = J(\boldsymbol{\mu})(\mathbf{x} - \boldsymbol{\mu}) + \mathbf{r}(\mathbf{x})$$
where $\norm{\mathbf{r}(\mathbf{x})} \leq C \norm{\mathbf{x} - \boldsymbol{\mu}}^2$ and $C = \frac{1}{2}\sup_{\mathbf{z} \in \mathcal{K}} \norm{D^2 \Phi(\mathbf{z})}$, which is finite by continuity on the compact set $\mathcal{K}$.

More directly: since $\Phi$ is $C^1$ on the compact convex hull of $\mathcal{K}$, by the mean value inequality,
$$\norm{\Phi(\mathbf{x}) - \Phi(\boldsymbol{\mu})} \geq \underline{\sigma} \cdot \norm{\mathbf{x} - \boldsymbol{\mu}}$$
whenever $\mathcal{K}$ is convex and the operators are injective on $\mathcal{K}$ (which follows from $\underline{\sigma} > 0$ via the constant-rank level set theorem). The per-operator bound follows from $\max_k \norm{\Phi_k(\mathbf{x}) - \Phi_k(\boldsymbol{\mu})} \geq \norm{\Phi(\mathbf{x}) - \Phi(\boldsymbol{\mu})}/\sqrt{K}$. $\square$

\vspace{0.5em}

\textbf{Theorem 3 (Finite-Sample Deviation Guarantee).} \textit{Let $\hat{\boldsymbol{\mu}}_n = \frac{1}{n}\sum_{i=1}^n \mathbf{x}_i$ be the sample mean from $n$ i.i.d.\ draws from $P_{\mathcal{N}}$ with population mean $\boldsymbol{\mu}$ and covariance $\Sigma$. Let each $\Phi_k$ be $L_k$-Lipschitz and let $L = \max_k L_k$. If the family is deviation-separating with $\underline{\sigma} > 0$, then with probability at least $1 - \delta$ over the draw of the sample:}

\begin{equation}
\max_{k} \norm{\Phi_k(\mathbf{x}) - \Phi_k(\hat{\boldsymbol{\mu}}_n)} \geq \frac{\underline{\sigma}}{\sqrt{K}} \norm{\mathbf{x} - \boldsymbol{\mu}} - \frac{L \sqrt{\text{tr}(\Sigma)} \cdot \sqrt{2 \ln(2m/\delta)}}{\sqrt{n}}
\label{eq:finite_sample}
\end{equation}

\textit{for any fixed anomaly $\mathbf{x} \in \mathcal{A}$.}

\textit{Proof.} Decompose the deviation at the estimated centroid:
\begin{align*}
\norm{\Phi(\mathbf{x}) - \Phi(\hat{\boldsymbol{\mu}}_n)} &\geq \norm{\Phi(\mathbf{x}) - \Phi(\boldsymbol{\mu})} - \norm{\Phi(\boldsymbol{\mu}) - \Phi(\hat{\boldsymbol{\mu}}_n)} \\
&\geq \underline{\sigma} \norm{\mathbf{x} - \boldsymbol{\mu}} - L \norm{\boldsymbol{\mu} - \hat{\boldsymbol{\mu}}_n}
\end{align*}
where the first term uses Theorem~2 (or Corollary~1 locally) and the second uses the Lipschitz property $\norm{\Phi(\mathbf{a}) - \Phi(\mathbf{b})} \leq L \norm{\mathbf{a} - \mathbf{b}}$.

It remains to bound $\norm{\hat{\boldsymbol{\mu}}_n - \boldsymbol{\mu}}$. Each coordinate $j$ of $\hat{\boldsymbol{\mu}}_n - \boldsymbol{\mu}$ has mean zero and variance $\Sigma_{jj}/n$. By Hoeffding's inequality applied to each coordinate with a union bound: with probability $\geq 1 - \delta$,
$$\norm{\hat{\boldsymbol{\mu}}_n - \boldsymbol{\mu}} \leq \frac{\sqrt{\text{tr}(\Sigma)} \cdot \sqrt{2\ln(2m/\delta)}}{\sqrt{n}}$$
(under sub-Gaussian tail assumptions on $P_{\mathcal{N}}$). Substituting and applying $\max_k \geq \norm{\cdot}/\sqrt{K}$ yields Eq.~\eqref{eq:finite_sample}. $\square$

\textbf{Corollary 2 (Sample complexity).} The finite-sample guarantee~\eqref{eq:finite_sample} is non-trivial (the right-hand side is positive) whenever:
\begin{equation}
n > \frac{2 L^2 K \cdot \text{tr}(\Sigma) \cdot \ln(2m/\delta)}{\underline{\sigma}^2 \cdot \norm{\mathbf{x} - \boldsymbol{\mu}}^2}
\label{eq:sample_complexity}
\end{equation}
This gives the minimum number of normal-class samples needed to reliably detect an anomaly at distance $\norm{\mathbf{x} - \boldsymbol{\mu}}$ from the true centroid.

\vspace{0.5em}

\textbf{Definition 2 (Operator Information Matrix).} For a deviation-separating family $\{\Phi_k\}$ at $\boldsymbol{\mu}$, define the \textit{total information matrix}:

\begin{equation}
\mathcal{I}(\boldsymbol{\mu}) := J(\boldsymbol{\mu})^\top J(\boldsymbol{\mu}) = \sum_{k=1}^{K} D\Phi_k(\boldsymbol{\mu})^\top D\Phi_k(\boldsymbol{\mu}) \in \R^{m \times m}
\label{eq:info_matrix}
\end{equation}

Note: $\mathcal{I}(\boldsymbol{\mu}) \succ 0$ if and only if the family is deviation-separating (equivalently, $\underline{\sigma} = \sqrt{\lambda_{\min}(\mathcal{I}(\boldsymbol{\mu}))} > 0$).

\vspace{0.5em}

\textbf{Theorem 4 (Information-Optimal Fusion Weights).} \textit{Consider the weighted action functional $S_{\mathbf{w}}[\mathbf{x}] = \sum_k w_k \norm{\Phi_k(\mathbf{x}) - \Phi_k(\boldsymbol{\mu})}^2$ with $\sum_k w_k = 1$, $w_k \geq 0$. The worst-case detection sensitivity}

\begin{equation}
\eta(\mathbf{w}) := \min_{\norm{\mathbf{v}}=1} \sum_{k=1}^K w_k \norm{D\Phi_k(\boldsymbol{\mu})\mathbf{v}}^2 = \lambda_{\min}\!\left(\sum_k w_k D\Phi_k(\boldsymbol{\mu})^\top D\Phi_k(\boldsymbol{\mu})\right)
\label{eq:sensitivity}
\end{equation}

\textit{is maximised by the weights:}

\begin{equation}
w_k^* = \arg\max_{\mathbf{w} \in \Delta_K} \lambda_{\min}\!\left(\textstyle\sum_k w_k D\Phi_k(\boldsymbol{\mu})^\top D\Phi_k(\boldsymbol{\mu})\right)
\label{eq:optimal_weights}
\end{equation}

\textit{where $\Delta_K$ is the probability simplex. This is a convex optimisation problem (maximising the minimum eigenvalue of a matrix affine in $\mathbf{w}$) and can be solved as a semidefinite programme (SDP).}

\textit{Proof.} The map $\mathbf{w} \mapsto \sum_k w_k D\Phi_k(\boldsymbol{\mu})^\top D\Phi_k(\boldsymbol{\mu})$ is affine in $\mathbf{w}$. The minimum eigenvalue of a symmetric matrix is a concave function (as an infimum of linear functions: $\lambda_{\min}(A) = \min_{\norm{\mathbf{v}}=1} \mathbf{v}^\top A \mathbf{v}$). Therefore $\eta(\mathbf{w})$ is concave on the compact convex set $\Delta_K$, and a global maximum exists. The maximisation can be cast as:
\begin{align*}
\max_{\mathbf{w},\, t} \quad & t \\
\text{s.t.} \quad & \textstyle\sum_k w_k D\Phi_k(\boldsymbol{\mu})^\top D\Phi_k(\boldsymbol{\mu}) \succeq t \cdot I_m \\
& \mathbf{w} \in \Delta_K
\end{align*}
which is a standard SDP. $\square$

\textbf{Proposition 4 (Trace-proportional heuristic).} \textit{When the per-operator information matrices $\mathcal{I}_k := D\Phi_k(\boldsymbol{\mu})^\top D\Phi_k(\boldsymbol{\mu})$ have approximately isotropic structure (i.e., $\mathcal{I}_k \approx c_k \cdot I_m$), the optimal weights simplify to:}

\begin{equation}
w_k^{\text{trace}} = \frac{\text{tr}(\mathcal{I}_k)}{\sum_{j=1}^K \text{tr}(\mathcal{I}_j)}
\label{eq:trace_weights}
\end{equation}

\textit{Proof.} Under the isotropy assumption, $\sum_k w_k \mathcal{I}_k \approx (\sum_k w_k c_k) I_m$, so $\lambda_{\min} = \sum_k w_k c_k$, which is linear in $\mathbf{w}$ and maximised on the simplex by allocating weight to the largest $c_k$. For a smooth allocation, $w_k \propto c_k = \text{tr}(\mathcal{I}_k)/m$. $\square$

\vspace{0.5em}

\textbf{Theorem 5 (Greedy Operator Selection).} \textit{Given a pool of $M$ candidate operators $\{\Phi_j\}_{j=1}^M$ and a budget $K < M$, the problem of selecting the $K$-subset $S^* \subset [M]$ that maximises the worst-case sensitivity:}

\begin{equation}
S^* = \arg\max_{S \subset [M],\, |S|=K} \lambda_{\min}\!\left(\textstyle\sum_{k \in S} \mathcal{I}_k\right)
\label{eq:selection}
\end{equation}

\textit{is NP-hard in general. However, the surrogate objective $f(S) = \log\det\!\left(\sum_{k \in S} \mathcal{I}_k\right)$ is monotone submodular, and the greedy algorithm (at each step, add the operator $j$ that maximises the marginal gain $f(S \cup \{j\}) - f(S)$) achieves:}
...
\textit{Proof.} The function $f(S) = \log\det(\sum_{k \in S} \mathcal{I}_k)$ is well-defined when the sum is positive definite. It is (i) \textit{monotone}: adding an operator adds a positive semidefinite term, so the determinant cannot decrease; (ii) \textit{submodular}: this follows from the matrix identity $\log\det(A + B) - \log\det(A) = \log\det(I + A^{-1/2}BA^{-1/2})$, which is a decreasing function of $A$ in the Loewner order (larger $A$ means smaller marginal gain from adding $B$). The $(1-1/e)$ guarantee then follows from the classical result of Nemhauser, Wolsey, and Fisher (1978) for greedy maximisation of monotone submodular functions. $\square$

\vspace{0.5em}

We now state the operational form used in the framework:

\textbf{Axiom (Operational Form).} Let $\epsilon_k$ be the $(1-\alpha)$ quantile of $\norm{\Phi_k(\mathbf{x}) - \Phi_k(\boldsymbol{\mu})}$ under $P_{\mathcal{N}}$ (the normal-class tolerance in law domain $k$). Then:
...
\textbf{Conditions.} The operational axiom holds under: (C1)~the law-domain operators $\{\Phi_k\}$ are deviation-separating (Definition~1); (C2)~the tolerances $\epsilon_k$ are calibrated to the normal distribution (false positive rate $\alpha$ per domain); (C3)~the anomaly $\mathbf{x}$ lies in the region where Theorem~1 applies (locally guaranteed; empirically verified globally). We validate the axiom on four real-world datasets in Section~\ref{sec:experiments}, where it holds with $K=5$ law domains and standard tolerance levels.
...
\textbf{Proposition 2 (Non-zero action for anomalies).} Under Theorem~1 and conditions (C1)--(C3), for any anomalous $\mathbf{x} \in \mathcal{A}$, $S[\mathbf{x}] > \min_k w_k \cdot \epsilon_k > 0$.

\textit{Proof.} By Theorem~1 and Eq.~\eqref{eq:axiom}, $\exists\, k$ such that $\norm{\Phi_k(\mathbf{x}) - \Phi_k(\boldsymbol{\mu})} > \epsilon_k$. Since all $w_k > 0$, the $k$-th term in $S[\mathbf{x}]$ satisfies $w_k \cdot \norm{\Phi_k(\mathbf{x}) - \Phi_k(\boldsymbol{\mu})} > w_k \cdot \epsilon_k > 0$, and all other terms are non-negative. $\square$
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{QuadSurf}: Polynomial surface calibration that fits a degree-2 surface over per-law magnitudes with weighted ridge regularisation, capturing non-linear interactions between law domains.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.85\columnwidth]{udl_figures/auc_vs_coverage.pdf}
...
Figure~\ref{fig:scatter} visualises the multi-law feature space by plotting the first two law-magnitude dimensions for three real-world datasets. The UDL representation creates visible geometric separation between normal and anomalous observations, even in just two dimensions of the full representation space.

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{udl_figures/projection_scatter.pdf}
\caption{2D projection of the UDL multi-law feature space for three real-world datasets. Red points (anomalies) separate from blue (normal) in the first two law-magnitude dimensions. On Shuttle, anomalies form a distinct cluster; on Mammography and Pendigits, anomalies are displaced along Law Dimension~1, consistent with the magnitude-dominated separation identified in the MDN analysis (Table~\ref{tab:mdn}).}
\label{fig:scatter}
\end{figure}
...
\begin{table*}[t]
\centering
\caption{Head-to-Head Comparison: UDL vs Established Methods (AUC-ROC). Baselines trained on normal data only; UDL-QDA-Magnified uses both-class labels. UDL-Hybrid uses 4~operators with cross-validated strategy selection.}
\label{tab:sota}
\begin{tabular}{@{}lcccccc@{}}
\toprule
\textbf{Method} & \textbf{Synth.} & \textbf{Mimic} & \textbf{Mamm.} & \textbf{Shuttle} & \textbf{Pendig.} & \textbf{Mean} \\
\midrule
\textbf{UDL-QDA-Magnified} & \textbf{1.000} & \textbf{1.000} & \textbf{0.947} & 0.989 & 0.777 & 0.943 \\
\textbf{UDL-Hybrid (4-op)} & 0.999 & 0.999 & 0.911 & \textbf{0.991} & \textbf{0.960} & \textbf{0.972} \\
\midrule
...
\begin{itemize}
  ...
  \item \textbf{UDL-QDA-Magnified retains the strongest performance on Mammography} (0.947), where the boundary-centred magnifier exploits the heterogeneous per-law sensitivity structure. The two UDL variants are complementary: QDA-Magnified excels when per-class covariance structure is well-defined; Hybrid excels when dimensionality or anomaly geometry varies across datasets.
...
\textbf{Extensibility.} The operator correlation audit (Section~\ref{sssec:redundancy}) demonstrates that adding new law domains yields non-redundant signal: among 91~operator pairs across 14~operators, only one pair exceeds $\rho > 0.85$. This validates the Representation Stack's (Eq.~\eqref{eq:stack}) open architecture---new $\Phi_k$ operators can be added with confidence that they contribute independent information.

\textbf{Coverage over AUC.} The coverage analysis (Section~\ref{sssec:coverage}) reveals that AUC alone is an insufficient deployment metric. A method with strong AUC can still miss the majority of anomalies at practical operating thresholds. We recommend that practitioners evaluate both AUC (ranking quality) and top-$k$ coverage (actionable detection rate), and use Hybrid auto-selection as the default strategy for robust coverage across domains.

\subsection{Limitations}

\begin{enumerate}
  \item \textbf{Supervised components}: The QDA-Magnified pipeline requires both-class labels for the magnifier and classifier stages. While the representation stack and centroid are unsupervised, the full pipeline is supervised. RankFusion provides a fully unsupervised alternative (mCov 89\%, no labels required).
  ...
  \item \textbf{Fisher projection fragility}: Fisher LDA collapses on high-dimensional data (Pendigits: 64 features, coverage drops to 13\%), likely due to ill-conditioned scatter matrices. The Hybrid auto-selection mitigates this by detecting Fisher failure via coverage cross-validation and switching to RankFusion.
  \item \textbf{Coverage--AUC divergence}: Methods with comparable AUC can exhibit dramatically different coverage profiles (Section~\ref{sssec:coverage}). We recommend always reporting both metrics.
  \item \textbf{Novelty channel}: The angular novelty component shows weak separation on the tested datasets. This may reflect insufficient diversity in the reference direction set.
...
\begin{enumerate}
  ...
  \item \textbf{Global extension of Theorem~1}: The local deviation guarantee (Theorem~1) is proved via the inverse function theorem. Extending to a global result for arbitrary anomaly sets $\mathcal{A}$ requires characterising operator families whose joint map is globally injective---\eg, affine operators, or operators satisfying a monotonicity condition.
...
\begin{enumerate}
  \item Formalises the Universal Deviation Principle as a theorem (Theorem~1): if the operator family is deviation-separating (full-rank stacked Jacobian), then no point near the centroid can match normal data across all law domains simultaneously. The local result is proved via the inverse function theorem; global validity is confirmed empirically.
  \item Implements this principle through 14~composable spectrum projection operators across five mathematical families, a tensor with MDN decomposition, and multiple projection strategies.
  \item Introduces a Boundary-Centred Dimension Magnifier paired with QDA for state-of-the-art AUC, and a Hybrid auto-selection strategy for state-of-the-art coverage.
  \item Achieves mean AUC-ROC of 0.972 (Hybrid) across five datasets with a single set of hyperparameters, \textbf{exceeding all established baselines} in fair head-to-head comparison. QDA-Magnified achieves the highest AUC on Mammography (0.947, $\Delta{=}+0.071$ vs $k$-NN Distance).
  \item Validates operator diversity through correlation analysis: among 91~operator pairs, only Wavelet$\leftrightarrow$Phase ($\rho{=}0.92$) is redundant---confirming that each mathematical perspective captures genuinely independent signal.
  \item Introduces top-$k$ \textbf{coverage} as a complementary metric, revealing that high AUC does not guarantee practical detection. Hybrid auto-selection achieves 91\% mean coverage (65\% worst-case), ensuring that most anomalies are surfaced at actionable thresholds.
  \item Detects camouflage-mimicking anomalies with AUC = 1.000 and demonstrates through cross-validation ($0.919 \pm 0.007$) that results are robust.
\end{enumerate}

The framework generalises BSDT's post-model correction into a universal, model-independent detection paradigm. By treating each observation as a multi-spectrum signal and measuring deviation across complementary mathematical laws, the Universal Deviation Law transforms anomaly detection from a single-metric threshold problem into a geometric structure analysis problem. The operator correlation analysis provides the strongest empirical evidence for the framework's central thesis: each mathematical perspective makes a genuinely independent contribution, and no single perspective is sufficient.
