...
Decentralised finance has upended much of what was settled about financial infrastructure. Transactions execute on blockchain networks without brokers, clearing houses, or custodians, which significantly reduces transaction costs and settles them in seconds instead of days or weeks, thereby eliminating entire classes of intermediaries. The efficiency dividend is apparent; however, the compliance deficit is very much real, especially for institutional investors---such as pension schemes, sovereign wealth funds, and asset management companies---who must adhere to stringent regulatory obligations.
...
% ============================================================
%  Fig. 1 — Four-Layer Architecture Diagram (TikZ) — ALIGNED
% ============================================================
\begin{figure*}[!t]
\centering
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
    ...
    layer/.style={draw, rounded corners=5pt, minimum width=17.6cm, minimum height=2.2cm,
                  fill=#1!8, thick},
    block/.style={draw, rounded corners=3pt, minimum height=0.85cm, minimum width=2.5cm,
                  fill=#1!22, font=\footnotesize, align=center, inner sep=5pt, thick},
    arrow/.style={-{Stealth[length=6pt]}, thick, gray!70!black},
    dasharrow/.style={-{Stealth[length=6pt]}, thick, dashed, red!70!black},
    lbl/.style={font=\footnotesize\bfseries, text=#1},
  ]

  % ===== Column x-coordinates (6 columns, evenly spaced) =====
  \def\colA{-7.0}   % col 1
  \def\colB{-4.2}   % col 2
  \def\colC{-1.4}   % col 3
  \def\colD{1.4}    % col 4
  ...
  % ===== Row y-coordinates (4 layers) =====
  \def\rowA{0}      % Layer I
  \def\rowB{-3.2}   % Layer II
  \def\rowC{-6.4}   % Layer III
  \def\rowD{-9.6}   % Layer IV

  % ---- Layer I background ----
  \node[layer=blue] (L1) at (0,\rowA) {};
  \node[lbl=blue!70!black, anchor=north west]
    at ([xshift=6pt, yshift=-4pt]L1.north west)
    {Layer I: Integration \& Interface};

  % ---- Layer I blocks (all at y = \rowA) ----
  \node[block=blue] (sdk)     at (\colA, \rowA) {TypeScript \&\\Python SDK};
  \node[block=blue] (rest)    at (\colB, \rowA) {RESTful API\\(:8000)};
  \node[block=blue] (auth)    at (\colC, \rowA) {Auth Gateway\\(JWT + RBAC)};
  \node[block=blue] (flutter) at (\colD, \rowA) {Flutter\\Consumer App};
  \node[block=blue] (nextjs)  at (\colE, \rowA) {Next.js\\War Room};
  \node[block=blue] (ws)      at (\colF, \rowA) {WebSocket\\Events};

  % ---- Layer II background ----
  \node[layer=green!70!black] (L2) at (0,\rowB) {};
  \node[lbl=green!50!black, anchor=north west]
    at ([xshift=6pt, yshift=-4pt]L2.north west)
    {Layer II: Compliance Logic};

  % ---- Layer II blocks (5 items → cols 1-5, centered) ----
  \node[block=green!70!black] (orch)      at (\colA, \rowB) {Orchestrator\\(:8007)};
  \node[block=green!70!black] (mlrisk)    at (\colB, \rowB) {ML Risk\\Engine (:8000)};
  \node[block=green!70!black] (graph)     at (\colC, \rowB) {Graph API\\(:8001)};
  \node[block=green!70!black] (sanctions) at (\colD, \rowB) {Sanctions\\Svc (:8002)};
  \node[block=green!70!black] (matrix)    at (\colE, \rowB) {Decision\\Matrix};

  % ---- Layer III background ----
  \node[layer=orange] (L3) at (0,\rowC) {};
  \node[lbl=orange!70!black, anchor=north west]
    at ([xshift=6pt, yshift=-4pt]L3.north west)
    {Layer III: Offline Training Pipeline};

  % ---- Layer III blocks (6 items → all 6 columns) ----
  \node[block=orange] (teacher)   at (\colA, \rowC) {Composite\\Teacher};
  \node[block=orange] (aexgb)     at (\colB, \rowC) {AE-XGBoost\\($w{=}0.4$)};
  \node[block=orange] (fatf)      at (\colC, \rowC) {FATF Patterns\\($w{=}0.3$)};
  \node[block=orange] (graphprop) at (\colD, \rowC) {Graph Props\\($w{=}0.3$)};
  \node[block=orange] (student)   at (\colE, \rowC) {Student\\Pipeline};
  \node[block=orange] (artefacts) at (\colF, \rowC) {Model\\Artefacts};

  % ---- Layer IV background ----
  \node[layer=red!70!black] (L4) at (0,\rowD) {};
  \node[lbl=red!60!black, anchor=north west]
    at ([xshift=6pt, yshift=-4pt]L4.north west)
    {Layer IV: Infrastructure \& Enforcement};

  % ---- Layer IV blocks (6 items → all 6 columns) ----
  \node[block=red!70!black] (nginx)     at (\colA, \rowD) {NGINX +\\Cloudflare};
  \node[block=red!70!black] (contracts) at (\colB, \rowD) {18 Contracts\\(Sepolia)};
  \node[block=red!70!black] (zknaf)     at (\colC, \rowD) {zkNAF\\(Groth16)};
  \node[block=red!70!black] (data)      at (\colD, \rowD) {Mongo / Redis\\/ Memgraph};
  \node[block=red!70!black] (ipfs)      at (\colE, \rowD) {IPFS /\\Audit Logs};
  \node[block=red!70!black] (docker)    at (\colF, \rowD) {17 Docker\\Containers};

  % ===== Inter-layer arrows (centered) =====
  \draw[arrow] (0, \rowA - 1.1) -- (0, \rowB + 1.1);
  \draw[arrow] (0, \rowB - 1.1) -- (0, \rowC + 1.1);
  \draw[arrow] (0, \rowC - 1.1) -- (0, \rowD + 1.1);

  % ===== Dashed arrow: model artefact → risk engine (deploy) =====
  \draw[dasharrow]
    (artefacts.south) -- ++(0,-0.55)
    -| (\colE, \rowB - 1.1)
    -- (matrix.south)
    node[pos=0.15, right, font=\scriptsize, text=red!70!black] {model deploy};
...
The Layer~II transaction processing system synthesises decisions. During data ingestion from Layer~I, the system checks the transaction data for risks using parallel Layer~II services concurrently, and the results are merged into one compliance decision.
...
\textbf{Stage~3b---Production Models (CPU Deployed).} This stage is responsible for deploying the production models (CPUs) that were created in Stage~3a. Docker volumes are used to mount the model artefacts into the Layer~II risk engine, where they will be used as risk measures.

% ============================================================
%  Fig. 2 — ML Pipeline Diagram (TikZ) — ALIGNED
% ============================================================
\begin{figure*}[!t]
\centering
...
\begin{tikzpicture}[
    every node/.style={font=\small},
    stage/.style={draw, rounded corners=3pt, minimum height=1.0cm, minimum width=2.6cm,
                  fill=#1!18, font=\footnotesize, align=center, inner sep=5pt, thick},
    data/.style={draw, rounded corners=2pt, minimum height=0.8cm, minimum width=2.4cm,
                 fill=gray!12, font=\scriptsize, align=center, inner sep=4pt, thick},
    arrow/.style={-{Stealth[length=5pt]}, thick},
    warrow/.style={-{Stealth[length=5pt]}, thick, blue!60},
    lbl/.style={font=\scriptsize, text=gray!60!black},
  ]

  % ===== Column positions (3 symmetric columns) =====
  \def\cL{-4.0}   % left
  \def\cM{0}      % centre
  \def\cR{4.0}    % right

  % ===== Row positions (top to bottom) =====
  \def\rData{0}        % data sources
  \def\rTeacher{-2.0}  % teacher components
  \def\rFusion{-4.0}   % fusion
  \def\rPseudo{-5.8}   % pseudo-labels
  \def\rStudent{-7.8}  % student pipeline
  \def\rMeta{-9.8}     % meta-learner
  \def\rDeploy{-9.8}   % deploy (same row, offset right)

  % ---- Row 1: Data Sources ----
  \node[data] (btc) at (\cL, \rData) {BitcoinHeist\\(Kaggle)};
  \node[data] (eth) at (\cM, \rData) {Ethereum\\(Kaggle)};
  \node[data] (bq)  at (\cR, \rData) {BigQuery\\(1,670 tx)};

  % ---- Row 2: Teacher Components (aligned below data) ----
  \node[stage=orange] (aexgb)  at (\cL, \rTeacher) {AE-XGBoost\\$w{=}0.4$};
  \node[stage=orange] (fatfp)  at (\cM, \rTeacher) {7 FATF Patterns\\$w{=}0.3$};
  \node[stage=orange] (graphp) at (\cR, \rTeacher) {Graph Properties\\$w{=}0.3$};

  % ---- Row 3: Fusion (centred) ----
  \node[stage=red!70!black] (fusion) at (\cM, \rFusion) {Composite Teacher\\Fusion};

  % ---- Row 4: Pseudo-labels (centred) ----
  \node[data] (pseudo) at (\cM, \rPseudo) {Pseudo-Labels\\(2.64\,M transactions)};

  % ---- Row 5: Student Pipeline (3 columns) ----
  \node[stage=blue] (vae)    at (\cL, \rStudent) {$\beta$-VAE\\(GPU)};
  \node[stage=blue] (gnn)    at (\cM, \rStudent) {GraphSAGE /\\GATv2 (GPU)};
  \node[stage=blue] (xgblgb) at (\cR, \rStudent) {XGBoost +\\LightGBM};

  % ---- Row 6: Meta-learner (centred) + Deploy (right) ----
  \node[stage=green!60!black] (meta) at (\cM, \rMeta) {Logistic Regression\\Meta-Learner};
  \node[stage=purple] (deploy) at (7.0, \rDeploy) {Docker Volume\\Deploy};
...
A single train--test split does not demonstrate how well a model generalises. For this reason, we also applied five-fold stratified cross-validation to the entire 625,168 unique address dataset (372 verified fraud addresses with fraud rates of 0.06\%). The Student-XGBoost base model results per fold are detailed in Table~\ref{tab:cv}.
...
All components are defined as infrastructure-as-code; this system will not just exist on paper or as a proposal, but will be an implementation ready to be used. The TypeScript and Python client SDKs are released as open source to facilitate third-party integration and independent audit.\footnote{Open-source SDKs: \url{https://github.com/segetii/fintech}.}
...
\textbf{Temporal graph analysis.} We want to ensure that we can continue to analyse transactions over time periods greater than one year so that we can factor in those evolving patterns when generating risk scores for new transactions.
