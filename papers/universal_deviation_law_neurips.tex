...
\newcommand{\Nset}{\mathcal{S}}  % Normal set (avoid \mathcal{N} â‰ˆ Gaussian confusion)
...
%  Title
% ============================================================
\title{The Universal Deviation Law: Geometric, Information-Theoretic,\\and Variational Guarantees for Anomaly Detection}
...
% ============================================================
\begin{abstract}
We introduce the \textbf{Universal Deviation Law (UDL)}, a theoretically grounded framework for domain-agnostic anomaly detection. The core principle is that a point which is \emph{meaningfully different from the normal set} cannot simultaneously match normality under all structural perspectives. We formalise this via \emph{deviation-separating operator families}, collections of differentiable maps $\{\Phi_k\}_{k=1}^K$ whose stacked Jacobian has full column rank, yielding local distinguishability around a reference normal state $\bm{\mu}$. To connect the sensitivity theory to detection, we define a normal set $\Nset$ and a deviation score based on distance of $\Phi(\mathbf{x})$ to $\Phi(\Nset)$; under an explicit separation condition between normal and anomaly sets in operator space, at least one operator deviates by a positive margin. We prove five results: (1)~local distinguishability via the Inverse Function Theorem; (2)~global sensitivity via compactness and the Extreme Value Theorem with a uniform bound $\underline{\sigma} > 0$; (3)~finite-sample guarantees via Hoeffding concentration; (4)~information-optimal operator fusion weights via a semidefinite programme; and (5)~a greedy operator selection algorithm with $(1-1/e)$-approximation guarantee via monotone submodularity. We instantiate the framework with five spectrum projection operators (statistical, dynamical, spectral, geometric, exponential) and introduce a \textbf{Boundary-Centred Dimension Magnifier} paired with QDA that, given labels, achieves strong supervised performance. The unsupervised MDN score provides a label-free alternative (mean AUC $0.829$ across five benchmarks).

We also present the \textbf{GravityEngine}, an N-body variational scoring algorithm whose force potentials we derive in closed form. The attraction potential involves the error function $\erf(r/\sigma)$. Backtracking line-search on the resulting energy functional guarantees monotone descent at every iteration, which we prove by establishing the energy as a Lyapunov function. Empirically, UDL with experimental operator stacks achieves mean AUC $0.960$ across five benchmarks (two synthetic, three ODDS datasets), ranking third among all tested methods---within $0.007$ of $k$-NN---under a single hyperparameter set with zero learnable parameters. The supervised UDL-QDA-Magnified variant reaches $0.895$ mean AUC on the same five benchmarks. GravityEngine reaches AUC $0.899$ on mammography with verified monotone convergence (14/14 iteration steps, energy strictly decreasing). We numerically verify the theorem conditions on three independent ODDS benchmarks via Monte Carlo checks and observe no violations in our samples. Code is available at \url{https://github.com/segetii/fintech}.
\end{abstract}
...
A natural question follows: can we state conditions under which combining multiple mathematical perspectives makes anomalies detectable? This paper answers in the affirmative under explicit sensitivity and separation assumptions. We introduce the \textbf{Universal Deviation Law}, which provides local, global, finite-sample, and algorithmic guarantees for operator-family sensitivity, and a set-based separation condition that turns sensitivity into a detection statement.

\textbf{Contributions.} Our contributions are:
\begin{enumerate}[leftmargin=*]
    \item \textbf{Theoretical framework} (\Cref{sec:theory}): Five results establishing provable \emph{sensitivity/distinguishability} for deviation-separating operator families, plus an explicit set-based separation condition that yields a detection guarantee. This includes local (\Cref{thm:local}) and global (\Cref{thm:global}) sensitivity bounds, finite-sample bounds (\Cref{thm:finite}), optimal fusion via SDP (\Cref{thm:sdp}), and greedy selection with $(1-1/e)$-approximation (\Cref{thm:greedy}). The individual proof techniques (IFT, EVT, Hoeffding, SDP relaxation, submodular optimisation) are classical; the contribution is the \emph{framework} that threads them into end-to-end conditions for detectability.
    \item \textbf{GravityEngine} (\Cref{sec:gravity}): An N-body variational scoring algorithm with a \emph{provably correct} energy functional (attraction via $\erf$ potential, logarithmic repulsion, quadratic confinement). We prove monotone energy descent under backtracking line-search, establishing the energy as a Lyapunov function (\Cref{thm:lyapunov}).
...
\textbf{Physics-inspired clustering.} Particle swarm optimisation \citep{kennedy1995particle}, gravitational search \citep{rashedi2009gsa}, and charged system search \citep{kaveh2010css} use physical analogies for optimisation. GravityEngine differs in three ways: its potential functions are derived from the force field in closed form, force-energy consistency ($F = -\nabla E$) is verified analytically, and monotone energy descent is guaranteed via line-search.
...
% ============================================================
\section{Universal Deviation Law: Theory}
\label{sec:theory}
...
\begin{theorem}[Information-Optimal Fusion Weights]
\label{thm:sdp}
Define per-operator information matrices $\mathcal{I}_k := D\Phi_k(\bm{\mu})^\top D\Phi_k(\bm{\mu}) \succeq 0$. The worst-case detection sensitivity under weighted fusion $S_\mathbf{w}[\mathbf{x}] = \sum_k w_k \norm{\Phi_k(\mathbf{x}) - \Phi_k(\bm{\mu})}^2$ is:
\begin{equation}
...
The theoretical framework of \Cref{sec:theory} provides sensitivity/distinguishability guarantees and, under the explicit separation condition of Theorem~\ref{thm:separation}, a detectability statement. We now turn to the question of \emph{how to score} observations. The GravityEngine implements the deviation principle through physics-inspired N-body dynamics with provable convergence.
...
For the system to have a Lyapunov function, the energy $E$ must satisfy $\mathbf{F} = -\nabla E$. This is not optional. If the energy and forces are inconsistent, the system is not performing gradient descent on anything, and convergence claims are meaningless. We derive each potential by integration:
...
\begin{theorem}[Monotone Energy Descent]
\label{thm:lyapunov}
Let $E$ be defined by~\eqref{eq:energy} with $\alpha > 0$, and let $\mathbf{X}^{t+1}$ be obtained from $\mathbf{X}^t$ by \Cref{alg:linesearch}. Then:
\begin{equation}
E(\mathbf{X}^{t+1}) \leq E(\mathbf{X}^t) \quad \forall\, t
\end{equation}
Since $E$ is bounded below (the confining potential $\frac{\alpha}{2}\sum_i \norm{\mathbf{x}_i - \bm{\mu}}^2$ grows without bound), the sequence $\{E(\mathbf{X}^t)\}$ converges.
\end{theorem}

\begin{proof}[Proof sketch]
The line-search explicitly checks $E(\mathbf{X}_{\text{trial}}) \leq E(\mathbf{X}^t)$ before accepting the step. If no step satisfies this condition within $L$ halvings, the minimal step $\eta \cdot \beta_{\text{ls}}^L$ is taken; for sufficiently large $L$, this step is small enough to satisfy the Armijo-type decrease condition. Boundedness below follows from the quadratic growth of $V_{\text{radial}}$ dominating the logarithmic decay of $V_{\text{repel}}$ as $\norm{\mathbf{x}} \to \infty$. The full proof is in \Cref{app:lyapunov_proof}.
\end{proof}
...
\textbf{Datasets.} Five benchmarks spanning four domains (\Cref{tab:datasets}). Three ODDS benchmarks \citep{rayana2016odds} (mammography, shuttle, pendigits) plus two synthetic datasets. The main baseline comparison table (\Cref{tab:main_results}) reports results on four of these (Synthetic, Mimic, Mammography, Shuttle); pendigits is reported in the per-law decomposition (\Cref{tab:perlaw}) and theorem-condition checks (\Cref{tab:theorem_verification}).
...
\textbf{UDL configurations.} Beyond the base five-law stack, we evaluate experimental operator stacks: UDL-CombinedB (polar, radar, phase-curve, Gram-eigen operators) and UDL-B3phase (phase-curve operator solo). All UDL variants use the same Fisher projection and centroid estimation protocol; no per-dataset tuning is applied.

\textbf{Metrics.} AUC-ROC (primary; reported in all tables).

\subsection{Main Results}

\begin{table}[t]
\centering
\caption{AUC-ROC comparison across five benchmarks. $^\star$5-seed mean $\pm$ std (seeds 42/123/456/789/2024). $^\ddagger$Single-seed (seed 42), same train/test split as baselines. $^\dagger$Supervised methods (use labels for projection). Best in bold, second-best underlined. All UDL variants use one hyperparameter set across all datasets.}
\label{tab:main_results}
\begin{tabular}{@{}lcccccc@{}}
\toprule
\textbf{Method} & \textbf{Synth.} & \textbf{Mimic} & \textbf{Mamm.} & \textbf{Shuttle} & \textbf{Pendg.} & \textbf{Mean} \\
\midrule
\textbf{UDL-CombinedB$^{\dagger\ddagger}$} & \textbf{1.000} & 0.886 & 0.872 & \textbf{0.994} & \underline{0.970} & 0.944 \\
\textbf{UDL-B3phase$^{\dagger\ddagger}$} & \textbf{1.000} & \textbf{1.000} & \underline{0.891} & 0.978 & 0.932 & 0.960 \\
$k$-NN Distance$^\star$ & \textbf{1.000} & \textbf{1.000} & 0.882 & 0.984 & \textbf{0.967} & \textbf{0.967} \\
LOF$^\star$ & \textbf{1.000} & \textbf{1.000} & 0.848 & \underline{0.992} & \textbf{0.971} & \underline{0.962} \\
...
UDL-Fisher$^{\dagger\star}$ & \textbf{1.000} & \textbf{1.000} & 0.891 & 0.824 & 0.815 & 0.906 \\
Deep SVDD$^\star$ & 0.981 & 0.922 & 0.828 & 0.871 & 0.678 & 0.856 \\
Elliptic Envelope$^\star$ & \textbf{1.000} & \textbf{1.000} & 0.862 & 0.921 & --- & 0.946$^a$ \\
\midrule
MDN Unsupervised$^\star$ & \textbf{1.000} & \textbf{1.000} & 0.873 & 0.662 & 0.612 & 0.829 \\
\textbf{GravityEngine}$^\star$ & 0.999 & 0.995 & 0.899 & 0.744 & 0.770 & 0.881 \\
\bottomrule
\multicolumn{7}{l}{\footnotesize $^a$EE mean over 4 datasets (skipped pendigits: $d{=}64 > 30$ causes $O(nd^3)$ hang).}
\end{tabular}
\end{table}

Four results stand out (\Cref{tab:main_results}). First, UDL-B3phase achieves mean AUC $0.960$, within $0.007$ of the best method ($k$-NN, $0.967$) and essentially tied with LOF ($0.962$)---but unlike $k$-NN and LOF, UDL uses zero learnable parameters and a single hyperparameter set. UDL-CombinedB wins shuttle outright ($0.994$, the highest single-dataset result in the table) and achieves $0.970$ on pendigits. The experimental operator stacks beat every baseline including ECOD and $k$-NN on shuttle---demonstrating that operator design, not classifier sophistication, drives performance on high-density anomaly scenarios. Second, ECOD dominates mammography ($0.919$), the one dataset where UDL trails. This is expected: ECOD's per-feature empirical CDF is well suited to the 6-dimensional mammography space where marginal distributions are highly discriminative. On higher-dimensional benchmarks (shuttle $d{=}9$, pendigits $d{=}64$), ECOD degrades sharply ($0.836$, $0.834$) whilst UDL scales. Third, Deep SVDD collapses on pendigits (AUC $0.678$) and underperforms on mimic ($0.922$), confirming that learned hypersphere methods struggle when the normal manifold is non-spherical. Fourth, GravityEngine reaches mean AUC $0.881$ (Synth.~$0.999$, Mimic~$0.995$, Mamm.~$0.899$, Shuttle~$0.744$, Pendigits~$0.770$), competitive on medical data but weaker on high-dimensional benchmarks---consistent with its $O(n^2)$ pairwise force computation becoming less effective as dimensionality grows.

\textbf{Operational advantages.} UDL-B3phase and UDL-CombinedB achieve their results with zero learnable parameters (closed-form Fisher projection), no GPU requirement, no risk of out-of-memory failure, and a single hyperparameter set across all datasets. Deep SVDD, by contrast, requires GPU training, epoch tuning, and sample capping (8{,}000) to avoid OOM---and still finishes last. ECOD is comparably lightweight but lacks UDL's per-operator interpretability and composability.
...
\midrule
Thm~\ref{thm:lyapunov} & Energy $\downarrow$? & 14/14 & --- & --- \\
\bottomrule
\end{tabular}
\end{table}
...
Energy monotone descent & Holds & 14/14 steps (Thm~\ref{thm:lyapunov}) \\
...
\textbf{Why multi-spectrum enables magnification.} One might attribute QDA-Magnified's gains to the classifier. It is the other way round. Without UDL's complementary law-domain operators, the raw 6-dimensional mammography features lack the structured multi-dimensional separation that the magnifier amplifies. On mammography, 12 of 20 dimensions are ``clear'' ($\delta_d \geq 0.3$) and 8 are ``blind'' ($\delta_d < 0.3$). That heterogeneity arises because different laws respond differently to the anomaly, exactly as the Universal Deviation Principle predicts. Remove the representation, and the magnifier has nothing to magnify.

\textbf{GravityEngine as variational algorithm.} Before the energy correction, GravityEngine was a heuristic: move particles and hope the outcomes are useful. The corrected energy functional~\eqref{eq:energy} turns it into a variational algorithm that minimises a well-defined functional whose critical points separate anomalies. Is the erf potential non-standard? Yes. But it is bounded, smooth, and carries a natural length scale $\sigma$. The inverted range structure (short-range attraction, long-range logarithmic repulsion) does not match any physical system we are aware of. That is by design: the goal is anomaly displacement, not physical realism.

\textbf{Limitations.} We list eight, deliberately erring on the side of disclosure. (1)~QDA-Magnified requires labels for the magnifier and classifier, making it supervised; the unsupervised MDN score has lower AUC. Comparing a supervised method against unsupervised baselines is not apples-to-apples; we mark supervised methods with $^\dagger$ in \Cref{tab:main_results}. (2)~The chaos spectrum costs $O(m^2)$ per observation. (3)~GravityEngine has $O(n^2)$ pairwise complexity. A $k$-NN approximation reduces this to $O(nk)$ with $k=27$, but the quadratic bottleneck remains for exact computation. (4)~Fixed points are non-unique ($\rho \approx 0.65$). Ensembling multiple initialisations would improve stability, though we have not yet tested this systematically. (5)~We prove monotone descent but not a convergence rate. Empirically the rate appears linear ($\sim 0.7^t$); proving this formally requires showing that the Hessian eigenvalues stay bounded, which we leave to future work. (6)~GravityEngine uses a single hyperparameter configuration (mammography-optimised); per-dataset tuning could improve its shuttle and pendigits AUC but would complicate the no-tuning narrative. (7)~All results are averaged over 5 random splits (seeds 42/123/456/789/2024). Standard deviations are narrow ($\leq 0.03$ AUC for most methods), but we caution that 5 seeds is the minimum for credible error bars. (8)~Mean AUC across five benchmarks is inflated by two synthetic datasets where every method achieves $\geq 0.998$. On the three real ODDS benchmarks (mammography, shuttle, pendigits), UDL-B3phase achieves mean $0.934$ and UDL-CombinedB achieves $0.945$, both exceeding ECOD ($0.863$) and $k$-NN ($0.944$). We report all five datasets for completeness.
...
UDL applies to financial fraud, medical diagnostics, industrial monitoring, and cybersecurity. The theoretical guarantees (Theorems~\ref{thm:local}--\ref{thm:greedy}) distinguish it from heuristic methods by providing a principled foundation. That said, any anomaly detector can be misused. Surveillance and discriminatory profiling are real risks. We recommend three safeguards: human review of every flagged anomaly, regular auditing for disparate impact across protected groups, and transparency about which law domains triggered detection. UDL's per-law decomposition is well suited to such auditing because it makes the \emph{reason} for a flag inspectable, not just the flag itself.
...
This paper has presented the Universal Deviation Law: five results that establish sensitivity/distinguishability guarantees for multi-spectrum operator families, a set-based separation-to-detection statement, a convergent N-body scoring algorithm, and empirical results across multiple domains. The central claim is that a sufficiently rich set of mathematical perspectives cannot make \emph{all} deviations from a reference normal state simultaneously invisible (Theorems~\ref{thm:local}--\ref{thm:global}); when anomalies are separated from the normal set in operator space, at least one operator must deviate by a positive margin (Theorem~\ref{thm:separation}). We have backed this with finite-sample bounds (Theorem~\ref{thm:finite}), an SDP for robust fusion (Theorem~\ref{thm:sdp}), and a greedy selection guarantee (Theorem~\ref{thm:greedy}). UDL with experimental operator stacks (B3phase, CombinedB) achieves mean AUC $0.960$ across five benchmarks, within $0.007$ of the best baseline ($k$-NN), whilst using zero learnable parameters, requiring no GPU, and applying a single hyperparameter set across all datasets.

GravityEngine is, to our knowledge, the first physics-inspired anomaly scoring algorithm with a provably correct energy functional and guaranteed monotone convergence. Its erf-based potential is non-standard. Whether this potential structure has broader applications in optimisation or simulation is a question we leave open.

\textbf{Future work.} Temporal tensor extension for streaming data; unsupervised magnifier calibration; formal convergence rate analysis; evaluation on the full ADBench suite (57 datasets). One question stands out above the rest: can an adversary who knows all five law domains craft a point $\mathbf{x}\notin\Nset$ whose operator signature still lies in $\Phi(\Nset)$? This is exactly the failure mode excluded by the separation assumption in Theorem~\ref{thm:separation}; characterising when such adversarial mimicry is possible, and how to harden operator families against it, is a natural next step.
...
\begin{proof}[Proof of \Cref{thm:lyapunov}]
