...
We also present the \textbf{GravityEngine}, an N-body scoring algorithm whose force potentials we derive in closed form. The attraction potential involves the error function $\erf(r/\sigma)$. With backtracking line-search, the energy decreases monotonically at every observed iteration; the descent guarantee holds conditional on the line-search finding a step within the halving budget. Empirically, UDL with experimental operator stacks achieves mean AUC $0.960$ across five benchmarks (two synthetic, three ODDS datasets), ranking third among all tested methods---within $0.007$ of $k$-NN---under a single hyperparameter set with zero learnable parameters. The supervised UDL-QDA-Magnified variant reaches $0.895$ mean AUC on the same five benchmarks. GravityEngine reaches AUC $0.899$ on mammography with empirically verified monotone energy decrease (14/14 iteration steps). We numerically verify the theorem conditions on three independent ODDS benchmarks via Monte Carlo checks and observe no violations in our samples. Code is available at \url{https://github.com/segetii/fintech}.
\end{abstract}
...
\textbf{GravityEngine as energy-inspired algorithm.} Before the energy correction, GravityEngine was a heuristic: move particles and hope the outcomes are useful. The corrected energy functional~\eqref{eq:energy} turns it into a descent algorithm on a well-defined functional whose critical points separate anomalies. The descent guarantee is conditional on the line-search budget being sufficient (\Cref{rem:convergence_scope}); we do not claim a global Lyapunov stability result. Is the erf potential non-standard? Yes. But it is bounded, smooth, and carries a natural length scale $\sigma$. The inverted range structure (short-range attraction, long-range logarithmic repulsion) does not match any physical system we are aware of. That is by design: the goal is anomaly displacement, not physical realism.
...
This paper has presented the Universal Deviation Principle: five conditional results that establish sensitivity/distinguishability guarantees for multi-spectrum operator families under explicit assumptions (A1--A3), a set-based separation-to-detection statement, an energy-inspired N-body scoring algorithm, and empirical results across multiple domains. The central claim is that, given a sufficiently rich operator family calibrated on normal data, anomalies cannot match normal behaviour across all operators simultaneously (Theorems~\ref{thm:local}--\ref{thm:global}); when anomalies are separated from the normal set in operator space, at least one operator must deviate by a positive margin (Theorem~\ref{thm:separation}). We have backed this with finite-sample bounds (Theorem~\ref{thm:finite}), an SDP for worst-case sensitivity maximisation (Theorem~\ref{thm:sdp}), and a greedy selection guarantee (Theorem~\ref{thm:greedy}). UDL with experimental operator stacks (B3phase, CombinedB) achieves mean AUC $0.960$ across five benchmarks, within $0.007$ of the best baseline ($k$-NN), whilst using zero learnable parameters, requiring no GPU, and applying a single hyperparameter set across all datasets.

GravityEngine is, to our knowledge, the first physics-inspired anomaly scoring algorithm with a closed-form energy functional and empirically verified monotone convergence under backtracking line-search. The descent guarantee is conditional (\Cref{rem:convergence_scope}), not a global Lyapunov stability result. Its erf-based potential is non-standard. Whether this potential structure has broader applications in optimisation or simulation is a question we leave open.
