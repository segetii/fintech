...
experiment of Section~\ref{sec:empirical}, with real FDIC call-report data the
static alignment is $\costheta = -0.335$ on raw snapshots (expected negative:
the gradient opposes the force in the energy landscape, pulling the system
back toward equilibrium).  In the out-of-sample backtest, the first alarm
fires at 2007-Q1, six quarters before the Lehman collapse, confirming that
the detection functional provides genuine early warning.\nThe ratio $c_2/c_1 \approx 1$, indicating tight equivalence.
...
The model leads the SRISK-proxy by $1$--$3$ quarters in both the GFC and
COVID windows, consistent with the energy-based early-warning mechanism:
the MFLS detects structural leverage buildup before market stress indices
react.  An out-of-sample backtest (training on data~$\le 2006$, testing
on~$2007$--$2009$) raises the first alarm at 2007-Q1, six quarters before
the Lehman collapse, with a 71.4\% hit rate during the crisis window.
COVID leads are strong ($+3$Q) because the MFLS captured pre-existing
fragility in the banking sector that amplified the exogenous shock.
The 2022 rate-shock episode shows contemporaneous detection, consistent
with its rapid-onset, non-credit-driven character.
Data: FDIC Statistics on Depository Institutions call-report filings
(SPECGRP 1--7), aggregated server-side by institution type;
no researcher-chosen weights or synthetic transformations.

\subsection{Granger Causality}

Table~\ref{tab:granger} reports Granger causality from MFLS to the SRISK-proxy
composite.  With real FDIC call-report data ($T = 140$ quarters, 1990--2024),
the linear Granger test does not reject the null at conventional levels.
This is expected: MFLS is computed from regulatory balance-sheet data
(FDIC SPECGRP aggregates, $N = 7$ institution types, $d = 6$ features),
while the SRISK-proxy is constructed from market-side stress indices
(STLFSI, VIX, HY spread).  The two measure different facets of systemic
risk --- structural leverage buildup versus market-implied distress.
The lead-lag table and OOS backtest above provide stronger evidence
for the early-warning channel, as they do not assume linearity.

\begin{table}[h]
\centering
\caption{Granger causality: MFLS $\to$ SRISK-proxy.
$H_0$: MFLS does not Granger-cause the composite stress index.
$T = 140$ quarters (1990--2024), FDIC call-report data.}
\label{tab:granger}
\begin{tabular}{rrr}
\toprule
...
With real FDIC call-report data ($T = 140$ quarters, $N = 7$ institution types),
the system spends 68.6\% of quarters above the critical manifold $\Cman$ at
$\alpha = 0.10$.  The fraction below $\Cman$ (31.4\%) corresponds to the
pre-boom quiet decade (1994--2003) used as the normal reference period,
confirming that the standardisation procedure correctly identifies the
baseline.  The system transitions above $\Cman$ during the credit expansion
of 2004--2007, remains elevated through the GFC, and briefly returns to
near-threshold levels before the 2022 rate shock.  Figure~\ref{fig:crisis}
...
crisis windows using FDIC call-report data.  The GFC 2008 window
($T=8$ quarters of elevated energy) produces a welfare gap
$\mathcal{L}_{\mathrm{inaction}} = 19.57$, consistent with the consensus
that the GFC imposed the largest welfare cost of any post-war financial event
(Romer and Romer, 2017; Jordà et al., 2013).  COVID 2020 shows zero welfare
loss in the model because the energy signal did not detect a gradual
leverage-driven buildup---COVID was an exogenous pandemic shock, not
a credit-cycle event.  The peak CCyB of 231 bps during COVID reflects
high instantaneous stress in the banking sector.
The 2022 rate-shock episode produces a modest welfare gap of 1.22,
with CCyB = 0 bps, indicating the rate shock was absorbed without
a systemic leverage spike.

\begin{table}[h]
\centering
\caption{Welfare calibration results by crisis window.
$\mathcal{L}_{\mathrm{inaction}}$: integral of energy gap between
no-intervention and optimal-policy trajectories.
\%CL: percentage loss in permanent consumption.
Peak CCyB: maximum implied buffer increment (basis points).
Data: FDIC SDI call reports, $T = 140$Q, $N = 7$ institution types.}
\label{tab:welfare}
\begin{tabular}{lrrrr}
\toprule
Crisis Window & Quarters & $\mathcal{L}_{\mathrm{inaction}}$ & \%CL & Peak CCyB (bps) \\
\midrule
GFC 2008 & 8 & 19.57 & 100.0 & 152 \\
COVID 2020 & 4 & 0.00 & 0.0 & 231 \\
Rate Shock 2022 & 6 & 1.22 & 100.0 & 0 \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{Notes.}  COVID $\mathcal{L} = 0$ because the MFLS did not
detect a preceding credit buildup---COVID was an exogenous shock, not a
leverage-driven crisis.  The 2022 CCyB = 0 bps reflects that $\gamma^*(t)$
was near zero in that window (the rate shock did not produce a systemic energy
spike in the FDIC call-report data).
