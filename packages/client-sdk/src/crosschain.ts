...
export class CrossChainService {
   ...
   */
  async getAggregatedRiskScore(address: string): Promise<CrossChainRiskScore> {
   ...
   */
  async isGloballyBlocked(address: string): Promise<boolean> {
   ...
   */
  async getRiskScoreFromChain(address: string, chain: ChainName): Promise<number> {
  ...
  async sendRiskScore(
    ...
    const tx = await this.contract.sendRiskScore(
      chainId,
      targetAddress,
      riskScore,
      adapterParams,
  ...
  async blockAddressGlobally(
    ...
    const tx = await this.contract.blockAddressGlobally(
      chainIds,
      targetAddress,
      reason,
      { value: feeWithBuffer }
  ...
  async unblockAddressGlobally(
    ...
    const tx = await this.contract.unblockAddressGlobally(
      chainIds,
      targetAddress,
      { value: feeWithBuffer }
    );
  ...
  getSupportedChains(): ChainName[] {
    return Object.keys(LZ_CHAIN_IDS) as ChainName[];
  }
 ...
 */
export function createCrossChainService(config: CrossChainConfig): CrossChainService {
