"""
Polish the AMTTP academic essay DOCX in place (on a copy).
...
DST = r"C:\Users\Administrator\Downloads\AMTTP_Polished.docx"
...
EVAL_PARAGRAPHS = [
    ...
    (
        ...
        "The V2 pipeline\u2019s Student models were trained on address-level "
        "aggregates (625,168 addresses, 71 features comprising 5 tabular, "
        "64 \u03b2-VAE latents, reconstruction error, and GraphSAGE score), "
        "yet production scoring must operate on individual incoming transactions. "
        "Profiling revealed a severe training\u2013serving skew: at inference "
        "time, only 5 of 71 features were computable from a raw transaction, "
        "with the remaining 66 features\u2014including all deep learning "
        "embeddings\u2014hardcoded to zero. Consequently, 93% of model "
        "inputs were zeros in production, degrading the Student v2 XGBoost "
        "to ROC-AUC of 0.6436 and F1 of 0.483 when evaluated on "
        "transaction-granularity data."
    ),

    (
        "To resolve this skew, AMTTP adopts Path B: retraining dedicated "
        "transaction-level models on 21 features that are fully available "
        "at inference time\u20146 extracted directly from the API "
        "TransactionRequest payload, 7 derived via arithmetic transformations, "
        "and 8 sender-history features retrieved from an address index cache "
        "(gracefully defaulting to zero if unavailable). Labels are propagated "
        "from address-level ground truth to individual transactions via sender "
        "join: if an address carries a fraud label, all of its outgoing "
        "transactions inherit that label, yielding 420,848 fraudulent "
        "transactions out of 1,673,244 total (25.15% fraud rate). "
        "Table VIII compares the resulting transaction-level models against "
        "the address-level baselines evaluated on the same transaction data."
    ),

    None,  # Placeholder for TABLE VIII — Cross-Level Comparison

    (
        "The results demonstrate that matching the training granularity to the "
        "inference granularity is decisive: TX-Level XGBoost achieves ROC-AUC "
        "of 0.9878 versus the address-level Student v2 XGBoost\u2019s 0.6436\u2014"
        "an absolute improvement of 0.344. F1 improves from 0.483 to 0.901. "
        "Crucially, the TX-Level models exhibit zero training\u2013serving skew: every "
        "feature used during training is guaranteed to be present at inference. The "
        "meta-learner reduces to a two-input logistic regression over XGBoost and "
        "LightGBM probabilities (coefficients 7.74 and 2.22, intercept \u22126.25), "
        "eliminating all GPU and graph-computation dependencies from the real-time "
        "scoring path."
    ),
    ...
    (
        "To assess generalization beyond the training distribution, AMTTP\u2019s V2 "
        "pipeline is evaluated against three independent external datasets using a "
        "multi-strategy protocol. For each dataset, five strategies are tested: (A) "
        "XGBoost with zero-padded feature alignment; (B) full pipeline with Platt "
        "recalibration on a 20%/80% calibration\u2013evaluation split; (C) Meta-LR "
        "with Platt scaling; (D) concept alignment, where the V2 score is appended as "
        "an additional feature to the external dataset\u2019s native features and "
        "evaluated via five-fold cross-validated logistic regression; and (E) the ML3 "
        "reinforcement learning module with Platt correction. Platt scaling fits a "
        "logistic regression on the calibration partition\u2019s predicted probabilities "
        "versus true labels and transforms all evaluation-partition scores accordingly, "
        "addressing distributional shift without any retraining of the base models."
    ),

    (
        "The Elliptic Bitcoin dataset (46,564 labelled transactions, 4,545 illicit, "
        "from law enforcement ground truth) provides the most rigorous cross-chain test: "
        "the student model was trained exclusively on Ethereum data pseudo-labelled by a "
        "teacher that itself was trained predominantly on Bitcoin (99.7% BitcoinHeist). "
        "The knowledge distillation thus crosses both the chain boundary and the labelling "
        "methodology boundary. The XBlock Ethereum Phishing dataset (9,841 addresses, "
        "2,179 confirmed phishing; Zhejiang University) constitutes a cross-level "
        "generalization test: address-level labels are scored by a transaction-level model, "
        "with address aggregates mapped to sender/receiver features. The Forta Network "
        "dataset (7,891 addresses, 268 malicious) reveals an instructive negative result: "
        "both student and teacher achieve ROC-AUC below 0.5, attributable not to model "
        "failure but to semantic mismatch between AMTTP\u2019s behavioural fraud "
        "patterns and Forta\u2019s intelligence-based threat labels. Distribution shift "
        "is also pronounced: training-set mean gas price is 1.1 gwei versus 42.4 for "
        "Forta addresses, and mean sender transaction count is 2,833 versus 40."
    ),

    (
        "Because external datasets supply only partial feature overlap, a feature "
        "reconstruction step is applied before scoring. For the XBlock dataset, the "
        "reconstruction pipeline computes seven FATF behavioural pattern scores "
        "(SMURFING, FAN_OUT, FAN_IN, LAYERING, STRUCTURING, VELOCITY, PEELING) from "
        "the available address aggregates, derives a sophisticated fraud score as their "
        "sum, and generates a hybrid risk score by blending the real Teacher XGBoost "
        "output with pattern and sophistication signals (0.4 \u00d7 Teacher XGB + "
        "0.3 \u00d7 pattern boost + 0.3 \u00d7 sophistication). This enrichment "
        "increases the TX-Level feature coverage from 9/21 to 14/21 and the Student "
        "v2 tabular coverage from 3/5 to 5/5. Evaluating before and after enrichment "
        "on XBlock demonstrates consistent improvement for the TX-Level models: "
        "TX-Level XGBoost ROC-AUC rises from 0.5833 to 0.6276 (+0.044), TX-Level "
        "LightGBM from 0.6385 to 0.6562 (+0.018), and TX-Level Ensemble from 0.6024 "
        "to 0.6454 (+0.043). F1 for the TX-Level Ensemble improves from 0.395 to "
        "0.416. The Student v2 models, by contrast, show ROC-AUC degradation after "
        "enrichment (Student XGBoost 0.6507 \u2192 0.3740), confirming that the "
        "address-level Student architecture with 66/71 features still zero-padded "
        "cannot be salvaged by marginal tabular enrichment alone."
    ),

    (
        "Adaptive percentile-based thresholds\u2014calibrated to each model\u2019s "
        "score distribution rather than using a fixed cut-off\u2014further unlock "
        "latent ranking ability. On XBlock with percentile thresholds, the Teacher "
        "XGBoost achieves ROC-AUC of 0.7492 and F1 of 0.5288 (at the 39.4th "
        "percentile), the best single-model result across all generations. This is "
        "striking because the Teacher\u2019s raw probabilities span only "
        "[0.000256,\u20090.000401], rendering any fixed threshold above 0.001 "
        "unusable; the percentile approach recovers its full discrimination power. "
        "On Elliptic, a concept alignment test appends the V2 fraud score as an "
        "additional feature to Elliptic\u2019s 166 native features and evaluates via "
        "five-fold cross-validated logistic regression: ROC-AUC rises from 0.9311 "
        "(native only) to 0.9334 (native + V2 score), and PR-AUC from 0.6241 to "
        "0.6481, confirming that the cross-chain signal adds incremental value even "
        "when the target dataset is already information-rich. A multi-layer coverage "
        "analysis on XBlock shows that combining ML, rule-based, and FATF pattern "
        "detectors achieves 99.8% recall on the 2,179 phishing addresses, with rules "
        "and FATF patterns uniquely catching 22 fraud cases that the best ML model "
        "missed."
    ),

    (
        "A small-sample external sanity check corroborates directional validity. "
        "Twenty-three Ethereum addresses\u2014three independently confirmed as "
        "fraudulent via Etherscan transaction analysis\u2014are scored by the "
        "production pipeline. At the production risk threshold (CRITICAL or HIGH), "
        "the system achieves precision of 1.0 and recall of 0.333 (F1 = 0.50), "
        "with zero false positives among the 20 legitimate addresses. The continuous-"
        "score ROC-AUC of 0.4417 on this 23-address set is not statistically "
        "interpretable at n = 23 but the perfect precision confirms that the system "
        "does not produce spurious alerts on clean addresses. A comprehensive "
        "external benchmark with larger independently labelled ground-truth sets "
    ...
    (
        "Several limitations should be acknowledged. First, the V2 pipeline\u2019s near-perfect "
        "test metrics (ROC-AUC 1.0000) are evaluated on proxy labels derived from the "
        "teacher\u2019s own weak supervision framework; they measure internal consistency "
        "rather than generalisation to independently verified ground truth. The honest "
        "cross-validated evaluation on 372 independently labelled fraud addresses yields "
        "the more conservative Student-XGBoost ROC-AUC of 0.9957. Second, the external "
        "Etherscan validation set comprises only 23 addresses (3 fraud); at this sample size, "
        "no statistically powered conclusions can be drawn. Third, cross-domain evaluation "
        "against the Forta Network yields ROC-AUC below 0.5 for both student and teacher, "
        "underscoring that behavioural fraud models do not transfer to intelligence-derived "
        "threat taxonomies without domain adaptation. Fourth, no persisted API "
        "latency benchmarks exist; throughput is estimated at 500+ transactions per second "
        "based on architecture capacity but has not been measured under sustained load. "
        "Fifth, the cross-level label propagation\u2014inheriting address fraud labels onto "
        "all sender transactions\u2014assumes that every transaction from a fraudulent "
        "address is itself fraudulent, an approximation that may over-label benign activity."
    ),
]
...
TABLE_VIII_DATA = {
    "caption": "VIII\nCross-Level Generalization: Address-Level vs. Transaction-Level Models",
...
def insert_eval_section(doc):
    ...
    table_queue = {3: TABLE_V_DATA, 7: TABLE_VI_DATA, 17: TABLE_VIII_DATA, 32: TABLE_VII_DATA}
    inserted = 0
