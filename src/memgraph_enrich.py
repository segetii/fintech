#!/usr/bin/env python3
"""
Memgraph enrichment helper - unified client supporting:
 - Bolt (mgclient) mode (default when memgraph_host provided)
 - HTTP proxy mode (when proxy_url provided)
Exposes MemgraphEnricher.query_batch_addresses(addrs: List[str]) -> Dict[addr, feature_dict]
"""
import os
import time
import json
...
class MemgraphEnricher:
    ...
    """
    def __init__(self, host: str = None, port: int = 7687, user: str = None, password: str = None,
                 proxy_url: str = None, api_key: str = None, timeout: int = 30):
        self.host = host
    ...
    # Bolt (mgclient) path
    # ---------------------------
    def _connect_bolt(self):
        if self.conn:
            return
        if not HAVE_MGCLIENT:
            raise RuntimeError("mgclient not installed")
        if self.user and self.password:
            self.conn = mgclient.connect(host=self.host, port=self.port, username=self.user, password=self.password)
        else:
            self.conn = mgclient.connect(host=self.host, port=self.port)
    ...
    def _query_http_proxy(self, addrs: List[str]) -> Dict[str, Dict[str, Any]]:
        ...
        url = f"{self.proxy_url.rstrip('/')}/batch_query"
