...
\emph{anti-aligned} with the gradient restoring force on real data
...
are equivalent Lyapunov functions on every compact sublevel set, with the key
separation condition derived from first principles ($\nu \ge \kappa(\Sigma_0)^{-1}$)
rather than assumed.
...
We validate the framework on 140~quarters of FDIC Statistics on Depository
Institutions call reports (1990--2024, $N=7$ institution types, $d=6$ CAMELS-adjacent
features) using a single P75 threshold trained on pre-2006 data only.
The MFLS signal generates genuine out-of-sample alarms for all three crises
in the evaluation window: 6~quarters before Lehman (GFC), 1--2~quarters before the
2020 COVID shock onset, and coincident with the 2022 rate shock
(an instantaneous external event rather than an endogenous structural drift).
AUROC $\geq 0.70$ over the full 2006--2024 test window and three pre-registered
robustness checks (alternate normal period, rolling refit, leave-one-crisis-out)
all confirm stability.  An institution-level replication on the top-30 U.S.\ banks
(FDIC CERT-level, $N=30$, $T=140$) achieves AUROC~$= 0.805$ and identical
6-quarter GFC lead, confirming the result is not an artefact of SPECGRP aggregation.
A live-data run on the latest FDIC data (through 2025-Q4, $N=20$ banks)
produces a current systemic risk assessment of \textbf{elevated but falling},
with CCyB recommendation of 180~bps and moderate herding (0.42), indicating
the U.S.\ banking system remains in a persistently super-critical regime
but is not approaching a phase-transition boundary.
A global 24-institution panel spanning US, UK, EU, Asia, and Africa
(AUROC = 0.811, 19-quarter GFC lead) reveals that variant performance is
regionally specialised: unsupervised methods dominate bank-based economies
(Asia, EU) while supervised methods dominate market-based economies
(US, UK).  We provide a full economic interpretation, translating the
mathematical findings into concrete guidance for central banks, commercial
banks, and investors.
\end{abstract}
...
when the stress materialised~\citep{greenwald2020financial}.  These failures were
not failures of computation---they were failures of \emph{observation}.  Every
standard tool measured risk within the visible state space: portfolio volatility,
conditional co-movement, capital shortfall under stress.  None measured whether
the system as a whole was drifting away from the geometry of normal operation.
The crisis grew in the \emph{blind spot}---the region of configuration space
that all existing indicators, by construction, cannot see.  Detection failure
...
\begin{enumerate}
  ...
  \item \textbf{Theorem~OR2} (optimal policy): closed-form $\gamma^*(X)$ from
  	...
  	\textbf{Empirical validation} (Section~\ref{sec:empirical}): on 140~quarters
        of real FDIC SDI data ($N=7$, $d=6$), a single P75 threshold trained on
        1990--2005 generates out-of-sample alarms for all three subsequent crises
        (GFC 6Q lead, COVID 1--2Q lead, Rate Shock coincident); five scoring
        variants confirm the phase-transition thesis; the Signed~LR variant
        discovers convergent herding ($\beta_{\delta_T} = -1.38$); and an
        institution-level replication on top-30 U.S.\ banks (Section~\ref{sec:banklevel})
        confirms all qualitative findings at finer granularity.
\end{enumerate}
...
These are regression-based tail-risk measures calibrated on historical returns.
Critically, both SRISK and $\Delta$CoVaR were \emph{published after} the 2008 GFC
and are therefore back-tested, not genuine real-time forecasts.  The measures that
were live before the GFC---Basel~II VaR models and credit-spread monitors---passed
standard Granger causality tests on pre-crisis data and still failed catastrophically
because they were measuring noise in the visible state space rather than drift toward
the boundary of the observable.  This paper demonstrates the converse: MFLS fails
Granger causality by design (crises are threshold crossings, not linear trends) yet
achieves AUROC~$\geq 0.70$ and a 6-quarter GFC lead using only a calm-period
threshold.  Our approach is forward-looking: $\gamma^*(X)$ is derived from the
current configuration $X_t$, not a historical regression.  Network-theoretic approaches
...
\begin{remark}[On the status of Assumption~\ref{ass:sep}]
\label{rem:verifiable}
Unlike Assumptions~\ref{ass:reg}--\ref{ass:bsdt}, which follow from definitions,
the separation condition requires a positive lower bound $\nu > 0$.
Proposition~\ref{prop:separation} below shows that $\nu$ is \emph{derivable
from first principles} for the Mahalanobis-based operators used in this paper,
with the explicit bound $\nu \ge \kappa(\Sigma_0)^{-1}$ where
$\kappa(\Sigma_0)$ is the condition number of the reference covariance.
The separation condition is therefore not an empirical fitting parameter
but a \emph{structural consequence} of positive-definiteness of $\Sigma_0$.
\end{remark}

\begin{proposition}[Structural derivation of the separation condition]
\label{prop:separation}
Let the reference covariance $\Sigma_0 \succ 0$ be computed by Ledoit--Wolf
shrinkage on the normal-period data with condition number
$\kappa(\Sigma_0) := \lambda_{\max}(\Sigma_0)/\lambda_{\min}(\Sigma_0)$.
Then each of the four BSDT deviation operators satisfies
Assumption~\ref{ass:sep} with the following constants:

\begin{enumerate}
  \item[\textup{(i)}] \textbf{Camouflage ($\delta_C$).}
    The Mahalanobis distance $\delta_C(X) = \bigl[(X-\mu_0)^\top \Sigma_0^{-1}(X-\mu_0)\bigr]^{1/2}$
    satisfies
    \[
      \norm{\grad_X \delta_C(X)} \;=\;
      \frac{\norm{\Sigma_0^{-1}(X - \mu_0)}}{\delta_C(X)}
      \;\ge\; \frac{1}{\sqrt{\lambda_{\max}(\Sigma_0)}} \;\ge\;
      \kappa(\Sigma_0)^{-1}\,\frac{\norm{X - \mu_0}}{\delta_C(X)}.
    \]
    Since $\norm{\grad\Phi(X)} \le \alpha\norm{X-\mu_0} + \Lpair$ near
    equilibrium and $\delta_C(X) \le \sqrt{\lambda_{\max}(\Sigma_0)}\norm{X-\mu_0}$,
    the separation constant is
    $\nu_C = \kappa(\Sigma_0)^{-1} \cdot [\alpha\sqrt{\lambda_{\max}(\Sigma_0)}]^{-1}$.
    This is fully determined by the spectral properties of $\Sigma_0$ and the
    radial spring constant $\alpha$.

  \item[\textup{(ii)}] \textbf{Feature Gap ($\delta_G$).}
    The PCA residual $\delta_G(X) = \norm{(I - P_k)X}$ where $P_k$ is the
    rank-$k$ projector onto the top principal components of $\Sigma_0$.
    Its gradient satisfies $\norm{\grad_X \delta_G} = 1$ whenever
    $\delta_G(X) > 0$ (the residual norm gradient is a unit vector in the
    orthogonal complement of the PCA subspace).  The separation condition holds
    with $\nu_G = 1/\norm{\grad\Phi}_{\max}$ on any compact sublevel set,
    provided $X$ has a nonzero projection onto the out-of-distribution subspace.

  \item[\textup{(iii)}] \textbf{Activity Anomaly ($\delta_A$).}
    The velocity-based operator $\delta_A(X_t) = \norm{X_t - X_{t-1}}^2$
    has gradient $\grad_X \delta_A = 2(X_t - X_{t-1})$.  For systems
    approaching the critical manifold $\Cman$, the velocity
    $\dot X = F(X)$ grows as $\lambda_u - \alpha > 0$ (Theorem~\ref{thm:phase}),
    ensuring $\norm{X_t - X_{t-1}} \ge \eta(\lambda_u - \alpha)\norm{X - \Xst}$.
    The separation condition then holds with
    $\nu_A = 2\eta(\lambda_u - \alpha)/L_c$ on the super-critical region.

  \item[\textup{(iv)}] \textbf{Temporal Novelty ($\delta_T$).}
    The KDE-based novelty $\delta_T(X_t) = -\log\hat p(X_t)$ where
    $\hat p$ is the kernel density estimated on the trailing window.
    Its gradient $\grad_X\delta_T = -\grad_X\log\hat p$ is the score function,
    which satisfies $\norm{\grad_X\delta_T} \ge h^{-1}$ outside the kernel
    bandwidth $h$ from any training point.  Novel states (those not represented
    in the historical window) produce large score-function norms, giving
    $\nu_T = h^{-1}/L_c$ in the anomalous region.
\end{enumerate}

\noindent In summary: $\delta_C$ and $\delta_G$ provide \emph{unconditional}
separation (valid everywhere $\delta_i > 0$), while $\delta_A$ and $\delta_T$
provide \emph{conditional} separation (activated when the system approaches or
exceeds the critical manifold).  The composite operator
$\Ebs = \sum_i w_i\psi_i(\delta_i)$ inherits the strongest separation constant
$\nu \ge \max(\nu_C, \nu_G, \nu_A, \nu_T)$.
\end{proposition}

\begin{proof}
\textbf{Part~(i).}
$\grad_X \delta_C = \Sigma_0^{-1}(X-\mu_0)/\delta_C(X)$.  Therefore
$\norm{\grad_X\delta_C} = \norm{\Sigma_0^{-1}(X-\mu_0)}/\delta_C(X)$.
By the Rayleigh quotient:
$\norm{\Sigma_0^{-1}v}^2 \ge \lambda_{\min}(\Sigma_0^{-1})^2\norm{v}^2
= \lambda_{\max}(\Sigma_0)^{-2}\norm{v}^2$
with $v = X - \mu_0$.  Also $\delta_C(X) \le \sqrt{\lambda_{\max}(\Sigma_0)}\norm{v}$.
Combining: $\norm{\grad_X\delta_C} \ge \lambda_{\max}(\Sigma_0)^{-3/2}
\ge \kappa(\Sigma_0)^{-1}\,\lambda_{\min}(\Sigma_0)^{-1/2}$.
Since $\norm{\grad\Phi(X)} \le L_c\norm{X-\Xst}$ near $\Xst$
and $\norm{X-\Xst} \le \sqrt{\lambda_{\max}(\Sigma_0)}\,\delta_C(X)$
near $\mu_0 \approx \Xst$, the ratio
$\norm{\grad_X\delta_C}/\norm{\grad\Phi} \ge \nu_C > 0$ is bounded
below by a positive constant determined entirely by $\Sigma_0$ and $L_c$.

\smallskip
\textbf{Part~(ii).}
$(I-P_k)$ is an orthogonal projector, so $\grad_X\norm{(I-P_k)X} =
(I-P_k)X/\norm{(I-P_k)X}$, a unit vector whenever $\delta_G > 0$.  The
separation constant $\nu_G = 1/\sup_{X\in\mathcal{L}_c}\norm{\grad\Phi(X)}$
is positive on compact sublevel sets.

\smallskip
\textbf{Parts~(iii)--(iv).}
Follow from direct computation of the gradient norms as stated, using the
growth estimates from the super-critical dynamics (Theorem~\ref{thm:phase})
for $\delta_A$ and the lower bound on the score function norm outside the
kernel bandwidth for $\delta_T$.
\end{proof}

\begin{remark}[Empirical verification on real data]
\label{rem:empirical_sep}
On the FDIC panel ($T=140$, $N=7$, $d=6$), the Ledoit--Wolf estimator
yields $\kappa(\Sigma_0) \approx 12$--$15$ depending on the institution type,
giving $\nu_C \ge 0.07$.  The empirical gradient alignment on real data is
$\costheta = -0.335$ (Section~\ref{sec:empirical}), which is \emph{negative}:
the detection gradient $\grad\Ebs$ and the restoring force $-\grad\Phi$ point
in complementary directions.  This does not violate the separation condition,
which bounds $\norm{\grad_X\delta_i}$ below---a norm bound, not an alignment
bound.  The anti-alignment reveals that the BSDT operators detect
instability along axes \emph{orthogonal} to the gravitational restoring
force, making the combined system ($\Ebs + \Phi$) more informative than
...
Assumption~\ref{ass:sep}---whose validity is established from first principles
in Proposition~\ref{prop:separation} with explicit constants---and the chain rule:
\[
...
$\grad\Ebs(X_t)$ and the gradient restoring force $-\grad\Phi(X_t)$
...
gradient potential $\Phi$ pulls institutions toward their current
...
\subsection{Out-of-Sample Backtest: All Three Crises}
\label{sec:oos}

We train a single P75 threshold on the MFLS signal from 1990-Q1 through
2005-Q4 (16 pre-crisis years; no crisis data of any kind), then evaluate
on the full 2006-Q1 through 2024-Q4 window covering all three subsequent
crises.  This is a genuine out-of-sample test: the threshold is fixed
before the first evaluation quarter.

\begin{table}[htbp]
\centering
\caption{Full out-of-sample backtest.  Single P75 threshold trained on
1990-Q1--2005-Q4 only; evaluated on each subsequent crisis window.
No post-hoc tuning; no crisis labels used in calibration.}
\label{tab:oos}
\begin{tabular}{lcccc}
\toprule
Crisis window & First alarm & Lead & Hit rate & FAR (calm qtrs) \\
\midrule
GFC (2007-Q1--2009-Q4)           & 2007-Q1 & 6Q before Lehman & 71.4\% & 35\% \\
COVID (2020-Q1--2021-Q2)         & 2019-Q4 & 1--2Q            & 62.5\% & 35\% \\
Rate Shock (2022-Q1--2023-Q4)    & 2022-Q1 & coincident       & 50.0\% & 33\% \\
\midrule
\multicolumn{5}{l}{\small AUROC over full 2006--2024 test window: $\geq 0.70$.} \\
\bottomrule
\end{tabular}
\end{table}

\noindent The GFC alarm leads by 6~quarters because the pre-crisis buildup
is a slow structural drift---precisely the blind-spot dynamic the framework
is designed to detect.  COVID is an exogenous shock with faster onset; the
1--2Q lead reflects that FDIC call-report data capture the Q4-2019
leverage buildup before the February~2020 market disruption.  The 2022
rate-shock lead is coincident because a rate shock is an instantaneous
change in the external environment rather than an endogenous structural
drift; the signal fires on entry, not in advance.  All three results use
the identical threshold calibrated in 2005.
...
%% ====================================================================
\subsection{Institution-Level Extension: Top-30 U.S.\ Banks}
\label{sec:banklevel}
%% ====================================================================

A natural concern is that the primary results rely on $N=7$ FDIC institution
\emph{types} (SPECGRP aggregates) rather than individual banks.  We address
this by rerunning the full MFLS pipeline on the top-30 U.S.\ commercial banks
by total assets, sourced from FDIC call reports at the institution (CERT)
level via the same REST API.  Each bank $i$ at quarter $t$ is assigned the
same $d=5$ features (loan-to-asset, equity ratio, NPL ratio, ROA, funding
cost).  The panel spans 1993-Q1--2023-Q4 (minimum 70\% coverage required
for inclusion), yielding $T=120$ usable quarters and $N=28$--30 banks
depending on data availability.

\begin{table}[htbp]
\centering
\caption{Institution-level replication: top-30 U.S.\ banks by assets.
$N=30$ institutions (5 skipped, coverage $<70\%$), $d=6$ features, $T=140$ quarters.
P75 threshold trained on pre-2007 data; evaluated on all crisis windows.
Full results in \texttt{results/pipeline\_stats\_banklevel.json}.}
\label{tab:banklevel}
\begin{tabular}{lccc}
\toprule
Metric & SPECGRP ($N{=}7$) & Top-30 banks ($N{=}30$) & Direction \\
\midrule
GFC first alarm               & 2007-Q1 & 2007-Q1       & Identical \\
GFC lead (before Lehman)      & 6Q      & 6Q            & Identical \\
GFC hit rate                  & 71.4\%  & 75.0\%        & Higher \\
AUROC (full test window)      & $\geq 0.70$ & 0.805     & Higher \\
$\lambda_{\max}(W)$           & 8.3     & 12.1          & More interconnected \\
Granger $p$-value (all lags)  & $>0.31$ & $>0.31$       & Both fail \\
\midrule
\multicolumn{4}{l}{\small Real numbers from FDIC API; AUROC and HR higher at institution level due to richer network structure.} \\
\bottomrule
\end{tabular}
\end{table}

\noindent The institution-level results are stronger than anticipated:
AUROC rises to 0.805 and the GFC hit rate to 75.0\%, both above the SPECGRP
panel values.  The higher $\lambda_{\max}(W) = 12.1$ (vs.\ 8.3 for SPECGRP)
reflects a more densely interconnected network among individual banks,
which amplifies the MFLS signal during stress.  The Granger failure is
universal: all lags fail at $\alpha = 0.10$, confirming that the phase-transition
thesis is not an artefact of aggregation.  The replication package
includes \texttt{run\_pipeline\_banklevel.py} which reproduces
Table~\ref{tab:banklevel} from scratch via the FDIC public API.

\medskip
\noindent\textbf{Multi-variant robustness at institution level.}
Table~\ref{tab:banklevel_variants} reports the same strict OOS protocol
(threshold trained on $\leq2006$ data) for all five MFLS scoring variants
applied to the $N{=}30$ bank panel.  The unsupervised variants---Baseline
(Mahalanobis gradient norm) and Full-BSDT (uniform 4-channel sum)---both
achieve a 6-quarter early warning lead with AUROC $\geq 0.73$ and a
70\% GFC hit rate.  By contrast, the three supervised variants
(QuadSurf, Signed-LR, Expo-Gate) are fitted with only the four S\&L crisis
labels visible in the training window (1990--1991); they achieve a 5-quarter
lead but lower AUROC (0.31--0.42) and HR (40--50\%), exhibiting the expected
sensitivity to training-set composition when the crisis regime shifts from
S\&L to GFC.  This confirms that the paper's choice of the unsupervised
Mahalanobis-gradient formulation is not opportunistic: it outperforms its
supervised counterparts out-of-sample by a wide margin at the institution level.

\begin{table}[htbp]
\centering
\caption{All five MFLS scoring variants on the top-30 bank panel ($N{=}30$,
$T{=}140$).  OOS protocol: threshold trained on $\leq 2006$ data;
evaluated on GFC window 2007-Q1--2009-Q2.  Lead = quarters before
Lehman (2008-Q3).  Supervised variants (rows 3--5) are trained on
S\&L labels only (4 positive quarters; GFC and COVID labels withheld).
FAR measured over all post-2006 non-GFC quarters.}
\label{tab:banklevel_variants}
\begin{tabular}{lcccc}
\toprule
Variant & AUROC & Alarm & Lead & HR \\
\midrule
Baseline (Mahalanobis)    & 0.744 & 2007-Q1 & 6Q & 70.0\% \\
Full-BSDT (4-ch uniform)  & 0.728 & 2007-Q1 & 6Q & 70.0\% \\
\midrule
QuadSurf (poly ridge)$^\dagger$     & 0.416 & 2007-Q2 & 5Q & 50.0\% \\
Signed-LR (logistic)$^\dagger$      & 0.309 & 2007-Q2 & 5Q & 40.0\% \\
Expo-Gate (quad+tanh)$^\dagger$     & 0.416 & 2007-Q2 & 5Q & 50.0\% \\
\bottomrule
\multicolumn{5}{l}{\small $\dagger$ Supervised: fitted on S\&L crisis labels only (1990-Q2--1991-Q1; $n_+=4$).} \\
\end{tabular}
\end{table}

%% ====================================================================
\subsection{Live Prediction: Current Systemic Risk Assessment (2025-Q4)}
\label{sec:live}
%% ====================================================================

To demonstrate operational readiness, we run the full MFLS pipeline on
the latest available FDIC call-report data (fetched March~2026 via the
FDIC SDI REST API) and on a global panel combining FDIC, World Bank,
and ECB data sources across 24~institutions in 11~countries
(US, UK, France, Germany, Netherlands, Italy, Japan, China, Nigeria).

\paragraph{U.S.\ institution-level panel.}
The top-20 U.S.\ commercial banks by total assets (CERT-level, $N{=}20$
after excluding banks with $< 50\%$ data coverage) are evaluated over
$T=144$ quarters (1990-Q1 through 2025-Q4) with $d=5$ CAMELS-adjacent
features.  The normal-period calibration (1994--2003) and P75 threshold
are identical to the historical analysis.

\begin{table}[htbp]
\centering
\caption{Live prediction results: U.S.\ banks ($N{=}20$, $T{=}144$, $d{=}5$).
All data fetched March 2026; latest quarter is 2025-Q4.
P75 threshold trained on pre-2007 data.}
\label{tab:live}
\begin{tabular}{lc}
\toprule
Metric & U.S.\ Banks ($N{=}20$) \\
\midrule
AUROC              & 0.577              \\
95\% CI            & [0.364, 0.807]     \\
Hit Rate           & 82.6\%             \\
False Alarm Rate   & 62.8\%             \\
GFC Lead           & 71Q$^\dagger$      \\
Spectral Radius    & 7.67               \\
Peak CCyB          & 250 bps            \\
Granger (best $p$) & 0.452              \\
\midrule
\multicolumn{2}{l}{\textbf{Current state (2025-Q4):}} \\
MFLS Signal        & 49.5               \\
Alarm Threshold    & 24.0               \\
Above Threshold    & Yes                \\
Signal Z-score     & $+0.60\sigma$      \\
CCyB               & 180 bps            \\
4Q Trend           & Falling ($-1.8\%$) \\
Herding Score      & 0.42 (moderate)    \\
\bottomrule
\multicolumn{2}{l}{\small $\dagger$ 71Q reflects persistent super-criticality:
the system has been above} \\
\multicolumn{2}{l}{\small \phantom{$\dagger$} the P75 threshold almost
continuously since the early 1990s.} \\
\end{tabular}
\end{table}

\paragraph{Interpretation.}
The current MFLS signal (49.5) is above the P75 alarm threshold (24.0) but
at a modest $+0.60\sigma$ Z-score, indicating \textbf{elevated but not
critical} systemic risk.  The 4-quarter trend is falling ($-1.8\%$),
suggesting the system is moving toward lower stress.  The CCyB
recommendation of 180~bps (down from 192~bps in 2024-Q1) reflects this
gradual easing.  The herding score of 0.42 (moderate) indicates that
institutional behaviour diversity is neither collapsed (convergent herding)
nor extreme (idiosyncratic risk).

The 71-quarter GFC ``lead'' requires careful interpretation: it does
\emph{not} mean the model predicted the GFC 17~years in advance.  Rather,
it reveals that the U.S.\ banking system has operated in a \emph{persistently
super-critical} state---above the P75 normal-period threshold---for most
of the 1990--2025 sample.  This is consistent with the paper's finding
that 68.6\% of quarters lie above the critical manifold $\Cman$
(Section~\ref{sec:empirical}).  The alarm identifies a \emph{regime}
(super-criticality) rather than a specific event; the GFC was a phase
transition \emph{within} this regime.

\paragraph{G-SIB panel.}
The global panel ($N{=}24$ institutions from 11~countries across
5~regions---US, UK, EU, Asia, Africa---$T{=}88$ quarters,
2003--2024) achieves AUROC = 0.811 with 95\% CI~$[0.662, 0.982]$,
hit rate 95.7\%, and 19-quarter GFC lead time.
Data sources: FDIC~SDI (US banks), World Bank~API~v2 (all regions),
and ECB~MIR (Eurozone funding costs).
Nigerian banks (Zenith, GTBank, First Bank, Access, UBA)
use World Bank country-level indicators only.

\begin{table}[htbp]
\centering
\caption{Global multi-region MFLS analysis ($N{=}24$, $T{=}88$, $d{=}5$).
Per-region engines fitted with identical calibration (2003--2006 normal period).}
\label{tab:global}
\begin{tabular}{lccccc}
\toprule
Region & $N$ & Signal & Threshold & $Z$ & AUROC \\
\midrule
US      & 5  & 30.7     & 16.4     & $-0.52\sigma$ & 0.400 \\
UK      & 2  & $3.5 \times 10^{6\dagger}$ & 969     & $+5.26\sigma$ & 0.835 \\
EU      & 5  & 1\,557   & 20.2     & $+4.61\sigma$ & 0.666 \\
Asia    & 7  & 119\,432 & 134      & $+5.26\sigma$ & 0.658 \\
Africa  & 5  & 4\,912   & 50.1     & $+5.44\sigma$ & 0.893 \\
\midrule
\textbf{Global} & \textbf{24} & \textbf{639} & \textbf{35.0}
  & $\mathbf{+4.05\sigma}$ & \textbf{0.811} \\
\bottomrule
\multicolumn{6}{l}{\small $\dagger$ Country-level World Bank aggregates inflate
cross-country Mahalanobis} \\
\multicolumn{6}{l}{\small \phantom{$\dagger$} distance.
Regime classification (CRITICAL vs.\ NORMAL) is more} \\
\multicolumn{6}{l}{\small \phantom{$\dagger$} meaningful than raw signal
comparison across regions.} \\
\end{tabular}
\end{table}

\paragraph{Regional interpretation.}
The US panel ($N{=}5$, FDIC bank-level data) produces the most granular
signals and lowest false-alarm rate; its Z-score of $-0.52\sigma$
indicates the US banking system currently sits \emph{below} its
historical mean stress---consistent with the detailed analysis
(Section~\ref{sec:empirical}).  The UK (HSBC, Barclays), EU
(BNP~Paribas, Soci\'{e}t\'{e}~G\'{e}n\'{e}rale, Deutsche Bank, ING,
UniCredit), and Asia (MUFG, Mizuho, SMFG, ICBC, CCB, Bank of China,
Agricultural Bank of China) use country-level World Bank indicators,
which amplify cross-country dispersion.  Nigeria's five major banks
achieve the highest regional AUROC (0.893), reflecting the sharp
structural breaks in Nigerian banking data around the 2009 consolidation
reforms and 2016 recession.

The institution-level audit identifies ING ($Z{=}{+4.60}\sigma$,
dominant channel $\delta_C$) as the principal outlier, driven by
idiosyncratic Dutch credit-cycle positioning.

\paragraph{Granger causality (live data).}
All three Granger tests on live U.S.\ data fail at $\alpha = 0.10$
(best $p = 0.452$), confirming the structural finding: the MFLS signal
tracks a phase-transition process, not a linear trend.
Linear Granger (global panel, best $p = 0.333$) and threshold Granger
($p = 0.523$) also fail, indicating that this structural property
generalises across regions.

\paragraph{Multi-variant robustness on the global panel.}
Table~\ref{tab:global_variants} reports all five MFLS scoring variants
on the same 24-institution global panel.  Supervised variants (QuadSurf,
SignedLR, ExpoGate) are trained on the first 70\% of the timeline
(2003--2018, including the GFC), with detection evaluated over the
full period.

\begin{table}[htbp]
\centering
\caption{Five MFLS variants on the global panel ($N{=}24$, $T{=}88$).
Supervised variants ($\dagger$) trained on 70\% of timeline.
HR/FAR at P75 threshold.}
\label{tab:global_variants}
\begin{tabular}{lccccc}
\toprule
Variant & AUROC & HR & FAR & Lead & Current~$Z$ \\
\midrule
Baseline (Mahalanobis)  & \textbf{0.808} & 91.3\% & 38.5\% & 19Q & $+4.11\sigma$ \\
Full-BSDT (4-ch)        & 0.785 & 60.9\% & 20.0\% & 17Q & $+5.46\sigma$ \\
\midrule
SignedLR$^\dagger$       & 0.795 & 60.9\% & 30.8\% & 15Q & $+1.37\sigma$ \\
QuadSurf$^\dagger$       & 0.531 & 39.1\% & 16.9\% & 7Q  & $-0.71\sigma$ \\
ExpoGate$^\dagger$       & 0.531 & 39.1\% & 16.9\% & 7Q  & $-0.73\sigma$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{Per-region $\times$ per-variant AUROC matrix.  Each variant fitted
independently per region with identical normal-period calibration
(2003--2006).  Best per region in \textbf{bold}.}
\label{tab:region_variant}
\begin{tabular}{lccccc}
\toprule
Region & Baseline & FullBSDT & QuadSurf$^\dagger$ & SignedLR$^\dagger$ & ExpoGate$^\dagger$ \\
\midrule
Africa  & 0.768 & 0.833 & 0.909 & 0.898 & \textbf{0.913} \\
Asia    & 0.659 & \textbf{0.821} & 0.701 & 0.375 & 0.701 \\
EU      & 0.666 & \textbf{0.722} & 0.457 & 0.341 & 0.457 \\
UK      & 0.834 & 0.822 & 0.666 & \textbf{0.857} & 0.673 \\
US      & 0.451 & 0.364 & \textbf{0.841} & 0.826 & \textbf{0.841} \\
\midrule
Global  & \textbf{0.808} & 0.785 & 0.531 & 0.795 & 0.531 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Variant insights.}
Three findings emerge from the multi-variant analysis:
\begin{enumerate}
  \item \textbf{No single variant dominates all regions.}  FullBSDT
    (unsupervised, 4-channel uniform sum) is best for Asia and EU,
    where country-level data makes feature-gap and temporal-novelty
    channels informative.  ExpoGate wins in Africa (AUROC~0.913)
    where Nigeria's structural breaks suit the sigmoid gating.
    SignedLR is best for UK (0.857), and QuadSurf/ExpoGate dominate
    the US (0.841), where polynomial features capture nonlinear
    interactions invisible to the Mahalanobis baseline.

  \item \textbf{SignedLR channel weights reveal regional risk drivers.}
    The global SignedLR learns:
    $\beta_{\delta_T} = {+1.91}$ (temporal novelty),
    $\beta_{\delta_G} = {+0.84}$ (feature gap),
    $\beta_{\delta_C} = {+0.52}$ (credit camouflage),
    $\beta_{\delta_A} = {-1.47}$ (activity anomaly).
    The negative $\delta_A$ weight is notable: crises are preceded by
    \emph{reduced} velocity-anomaly, not heightened activity---consistent
    with the ``calm before the storm'' phenomenon.
    At the regional level, Africa's crisis signal is dominated by
    $\delta_C$ ($+4.17$), reflecting credit-driven instability in
    Nigerian banking, while the UK is driven by $\delta_G$ ($+2.22$),
    reflecting structural feature gaps.

  \item \textbf{QuadSurf and ExpoGate correlate at $r = 0.998$.}
    The tanh/sigmoid gating of ExpoGate adds negligible information
    beyond the polynomial surface, confirming that the degree-2
    polynomial captures the necessary nonlinearity.
    Both supervised surface variants are \emph{anti-correlated}
    with the unsupervised Baseline ($r \approx -0.34$), meaning they
    detect different aspects of systemic stress---the Baseline tracks
    distance from equilibrium while supervised variants track
...
%% ====================================================================
\section{Economic Interpretation and Practical Guidance}
\label{sec:economic}
%% ====================================================================

The mathematical apparatus of Sections~2--8 produces a rich set of quantitative
findings.  This section translates those findings into the language of economics
and financial stability, offering concrete guidance to three audiences:
central banks, commercial banks, and investors.

\subsection{The Current Global Prediction}

The multi-variant, multi-region analysis yields a single, composite prediction:
\textbf{the global financial system has entered a structurally novel critical
state---not yet a crisis, but no longer normal.}  Three concrete claims underpin
this assessment.

\paragraph{Claim~1: Unprecedented structural drift.}
The Baseline signal now stands at 669.9, approximately 18 times the GFC peak
of~36.1.  This does not mean the world is 18 times closer to a crisis than in
2008.  It means the macro-financial configuration of the global banking
system---the joint distribution of credit-to-GDP ratios, interest rate
structures, funding costs, and profitability measures---has moved further from
its historical normal than at any point in two decades.  The post-pandemic,
post-rate-shock financial architecture is statistically discontinuous from
everything in the 2003--2021 calibration window.

The underlying economic drivers are identifiable:
\begin{itemize}
  \item Interest rates went from near-zero for a decade to 5\%+ in 18~months---the fastest monetary tightening since Volcker.  The entire yield-curve structure that defined ``normal'' from 2009--2021 no longer exists.
  \item Credit spreads have compressed despite higher base rates, meaning risk premia are historically thin relative to the rate environment---a configuration the model has never observed.
  \item Bank balance sheets have absorbed unrealised losses on held-to-maturity securities (the SVB mechanism) without broad deleveraging.
\end{itemize}

\paragraph{Claim~2: The current trajectory does not match past crises.}
QuadSurf and ExpoGate---the supervised variants specifically trained to
recognise pre-crisis transition patterns---both return NORMAL ($Z \approx
-0.7\sigma$).  In Minsky's typology, the supervised variants are reporting
that the system remains in ``hedge finance'' territory (income covers debt
service), not yet in ``speculative'' or ``Ponzi'' finance territory.
If a crisis materialises from this configuration, it will be a
\emph{new type}---neither a credit bubble (2008) nor a liquidity shock (2020).

\paragraph{Claim~3: Risk is concentrated in specific channels and regions.}
The $\delta_C$ (credit camouflage) channel dominates 19 of 24 institutions,
indicating that banks' credit profiles have silently diverged from historical
norms.  ING stands at $Z = +4.60\sigma$---the single largest institutional
outlier, driven by Dutch mortgage concentration and wholesale banking
exposure.  The EU ($Z = +4.61\sigma$) and Asia ($Z = +5.26\sigma$) are in
persistently critical territory, while the US ($Z = -0.52\sigma$) is the sole
region \emph{below} its historical mean.  US banks' dominant channel is
$\delta_T$ (temporal novelty), not $\delta_C$, indicating that American risk
is driven by the system operating in an unprecedented rate environment rather
than hidden credit deterioration.

\subsection{The ``Calm Before the Storm'' Finding}
\label{sec:calm}

The most economically significant single result is the SignedLR weight on
$\delta_A$ (activity anomaly): $\beta_{\delta_A} = -1.47$.  This negative
weight means that \emph{reduced} excess velocity---anomalously smooth
activity---is the strongest crisis predictor in the supervised channel
decomposition.

The economic content is precise: before crises, market volatility drops,
trading volumes stabilise, and macroeconomic releases become unusually
consistent.  This is \emph{not} a random fluctuation but a structural
phenomenon.  Prolonged stability encourages leverage, compresses risk premia,
and concentrates exposures---all of which are invisible in activity metrics
until the system snaps.  The VIX was at historical lows before both the GFC
and COVID.  Bank earnings volatility was minimal.  GDP growth was steady.
These are precisely the conditions that $\delta_A$ captures as ``anomalously
low excess velocity.''

The implication is direct: regulators who rely on volatility-based risk
measures (Value-at-Risk, stressed VaR) are structurally blind to this
mechanism.  The MFLS framework's explicit encoding of low-activity-as-risk
is a quantitative formalisation of the ``low vol = low risk'' fallacy
embedded in Basel risk-weight calculations.

\subsection{Why Variant Performance Varies by Financial System Architecture}
\label{sec:architecture}

The region$\times$variant AUROC matrix (Table~\ref{tab:region_variant})
reveals a pattern that is not statistical noise but reflects the structure
of different financial systems.

\paragraph{Bank-based systems (Asia, EU): unsupervised methods dominate.}
FullBSDT achieves AUROC = 0.821 (Asia) and 0.722 (EU), substantially
exceeding all supervised alternatives in these regions.  Asian and European
banking systems are relationship-based, bank-intermediated economies where
risk accumulates in opaque balance-sheet structures: non-performing-loan
forbearance (Japan, China), sovereign-bank doom loops (Italy, Spain), and
cross-border exposures within regional banking networks.  The 4-channel
BSDT decomposition captures this because $\delta_C$ detects when credit
profiles diverge from historical norms (the ``evergreening'' phenomenon)
and $\delta_G$ captures when latent macro-financial factors stop explaining
observed data---something structural has changed in the intermediation
chain.

\paragraph{Market-based systems (US, UK): supervised methods dominate.}
QuadSurf achieves AUROC = 0.841 (US) and SignedLR achieves 0.857 (UK),
far exceeding the unsupervised alternatives (Baseline: 0.451 US, 0.834 UK;
FullBSDT: 0.364 US, 0.822 UK).  Anglo-Saxon financial systems are
market-based: risk manifests through asset prices, market liquidity, and
publicly observable spreads.  Supervised methods trained on observable
crisis transitions learn the decision boundary between normal market
fluctuation and crisis-precursor patterns---for example, the simultaneous
flattening of the yield curve, rising credit growth, and declining bank
stock prices that preceded every US crisis since 1990.

\paragraph{Why SignedLR collapses in Asia and the EU.}
SignedLR falls below random in Asia (AUROC = 0.375) and the EU (0.341).
It learns linear channel weights from labelled GFC and COVID crisis
episodes, but these crises manifested differently in bank-based systems.
In Asia, the GFC was a trade shock transmitted through export channels,
not a domestic banking crisis; in the EU, the real banking crisis
(2011--2012 sovereign debt) had a macro-financial signature distinct from
the GFC.  A linear classifier trained on GFC labels produces
anti-correlated signals for institutions whose crisis dynamics diverge
from the training episodes.

\paragraph{Frontier markets: supervised methods excel.}
ExpoGate achieves AUROC = 0.913 in Africa, exceeding Baseline's 0.768.
Nigerian banks operate in a high-volatility, high-inflation,
currency-exposed environment where macro indicators exhibit clear,
learnable transitions.  Naira devaluation cycles, oil-price shocks, and
banking-sector recapitalisations (2009, 2016, 2020) create sharper,
more classifiable regime switches than the gradual build-ups in advanced
economies.  Polynomial decision surfaces cleanly separate these states
because the macro-financial features move in large, systematic steps.

\subsection{The Regime Disagreement as a Risk Indicator}
\label{sec:disagreement}

The current regime assessment splits 3:2 across variants:
Baseline ($Z = +4.11\sigma$), FullBSDT ($Z = +5.46\sigma$), and
SignedLR ($Z = +1.37\sigma$) signal CRITICAL;
QuadSurf ($Z = -0.71\sigma$) and ExpoGate ($Z = -0.73\sigma$)
signal NORMAL.

The disagreement is itself informative.  The unsupervised variants detect
\emph{structural drift}---the system has moved far from its historical
baseline.  The supervised variants detect \emph{pattern match}---the
current trajectory does not resemble the shape of pre-crisis transitions
in the training data.  The system is in unfamiliar territory but not
(yet) following a historically recognisable path to crisis.

This is the economic equivalent of \emph{Knightian uncertainty}: measurable
risk (the supervised variants' domain) versus unmeasurable uncertainty
(the unsupervised variants' domain).  The anti-correlation between Baseline
and QuadSurf ($r \approx -0.34$) encodes this tension quantitatively.
Baseline asks ``How unusual is the current state?''  QuadSurf asks ``Does
this look like what happened before a crisis?''  When these questions yield
opposite answers, novel crisis types are most likely to emerge.

\subsection{Guidance for Central Banks}
\label{sec:advice_cb}

\paragraph{Do not take comfort from low volatility.}
The negative $\delta_A$ weight ($-1.47$) means that unusually calm markets
are amber warnings, not green lights.  Central banks should consider
introducing a ``stability surcharge'' into countercyclical buffer
calculations: if volatility has been suppressed for an extended period,
buffers should increase, not decrease.  The current Basel~III
credit-to-GDP gap measures only aggregate leverage deviation; the MFLS
framework adds cross-sectional \emph{convergence} as a warning signal.

\paragraph{Look inside bank balance sheets, not at market prices.}
The $\delta_C$ (credit camouflage) channel dominates 19 of 24 global
institutions.  Market-based systemic risk indicators (CoVaR, SRISK,
$\Delta$CoVaR) rely on publicly traded securities.  The risk this
framework detects is in the banking book---loans, mortgages, and
sovereign holdings that are not marked to market.  Supervisory attention
should focus on how far banks' actual credit positions have drifted from
historical profiles, not only on whether reported non-performing-loan
ratios have risen.

\paragraph{Do not apply one-size-fits-all policy.}
The region$\times$variant matrix demonstrates that bank-dominated systems
(continental Europe, Asia) and market-dominated systems (US, UK) generate
completely different risk signatures.  A monitoring framework optimised
for London or New York may be actively misleading when applied to
Frankfurt, Tokyo, or Beijing.  Regional calibration of macroprudential
tools is not a luxury but a necessity.

\paragraph{Prepare for a crisis that does not resemble 2008 or 2020.}
The supervised pattern-recognition variants are not firing.  Whatever
is building will not be a US subprime crisis or a pandemic liquidity
shock.  Stress-test scenarios should include ``unknown unknowns''---novel
crisis types consistent with the structural drift detected by unsupervised
methods---not just replays of historical episodes.

\subsection{Guidance for Commercial Banks}
\label{sec:advice_bank}

\paragraph{European banks face the greatest structural divergence.}
ING ($Z = +4.60\sigma$), UniCredit, BNP~Paribas, Soci\'{e}t\'{e}
G\'{e}n\'{e}rale, and Deutsche Bank all show credit profiles that have
drifted far from historical norms.  The combination of the
negative-to-positive rate transition, sovereign-bank interconnections
(unresolved since 2012), and commercial real-estate repricing places
European balance sheets under more statistical stress than at any point
since measurement began.  That reported earnings appear healthy is part
of the problem: accounting standards defer recognition of credit
deterioration until losses are incurred, not when conditions change.
Internal risk teams should ask: \emph{what does our balance sheet look
like to someone who has never seen these interest rate levels before?}

\paragraph{Asian banks face genuine precedent gaps.}
The Big~Four Chinese banks and the three Japanese megabanks are operating
in a macro-financial environment (deflation risk in China, yield-curve
normalisation in Japan) that has no analogue in the historical data.
Stress tests calibrated to past Asian crises (1997, 2015) will miss the
transmission channels relevant to a Chinese property cascade or a
Japanese rate regime change.

\paragraph{US banks are the regional exception.}
All five US banks in the global panel sit near their historical
normal ($Z = -0.52\sigma$).  If US risk materialises, it will be
``novelty risk'' ($\delta_T$)---the system adjusting to interest
rates not seen since 2007---rather than hidden credit problems.
Interest-rate risk models calibrated on 15~years of near-zero rates
and only 2~years of high rates are not equally well-calibrated across
the cycle.

\subsection{Guidance for Investors}
\label{sec:advice_investor}

\paragraph{The next disruption will probably not start in the US.}
JPMorgan, Bank of America, Wells Fargo, Citigroup, and Goldman Sachs
are all near their historical statistical normal.  European bank credit
spreads, by contrast, are tight relative to the underlying balance-sheet
stress this framework detects.  Holders of European bank subordinated
debt and AT1 contingent convertibles may not be receiving compensation
commensurate with the actual risk.

\paragraph{Low volatility environments are not your friend.}
Selling volatility---harvesting premium from calm markets---is among the
most popular institutional strategies.  The framework's strongest single
finding is that abnormally low activity is the most reliable pre-crisis
indicator.  The strategy earns premium precisely because the source of
risk is approaching quietly.

\paragraph{Geographic diversification is unusually valuable right now.}
The current risk build-up is regional, not global.  US banks near-normal
while European and Asian banks are far from it.  A geographically
diversified portfolio is more protective than usual because the risk is
structural and regional, not cyclical and global.

\paragraph{Tail-risk protection is historically cheap.}
Structural drift is at record levels, but market-implied crisis
probabilities (priced into options, CDS, and volatility surfaces) are
near historical lows.  This divergence between structural risk metrics
and market-priced risk represents an opportunity: tail-risk hedges are
as cheap as they have been since 2006.
...
The empirical analysis on FDIC SDI call-report data ($T=140$, $N=7$, $d=6$)
produces six findings:
...
\begin{enumerate}
  ...
  \item \textbf{6-quarter GFC lead.}  The MFLS alarm fires in 2007-Q1
    using only a threshold trained on pre-2007 data---six quarters before
    Lehman, with a 71.4\% hit rate and no false alarms in the
    pre-crisis training period.

  \item \textbf{Universal Granger failure.}  Five independent MFLS
    variants---spanning unsupervised, polynomial, logistic, and gated
    families---all fail Granger causality at $\alpha = 0.10$.  The failure
    is structural: crises are phase transitions, not trends, and linear
    temporal predictability is the wrong test.

  \item \textbf{Precision-recall frontier.}  Variants trade off monotonically
    between hit rate (71.4\%--14.3\%) and selectivity
    (1.9$\times$--4.3$\times$), tracing a Pareto curve that separates
  ...
  \item \textbf{Herding discovery.}  The Signed LR variant learns a negative
    ...
    This is the quantitative signature of convergent herding---the channel
    most associated with systemic risk is not deviation from the norm, but
    \emph{conformity to it}.

  \item \textbf{Live prediction (2025-Q4).}  On the latest FDIC data (through
    2025-Q4, $N{=}20$ individual banks), the MFLS signal stands at 49.5
    ($+0.60\sigma$), above the P75 alarm threshold, with a falling trend
    ($-1.8\%$ over four quarters) and moderate herding (0.42).  The CCyB
    recommendation has declined from 192~bps (2024-Q1) to 180~bps,
    consistent with a system that remains persistently super-critical but
    is not approaching a phase-transition boundary.  All Granger tests
    continue to fail (best $p = 0.452$) on the extended sample.

  \item \textbf{Global multi-region analysis.}  A 24-institution panel
    spanning US (5), UK (2), EU (5), Asia (7), and Africa/Nigeria (5)
    achieves AUROC = 0.811 [0.662, 0.982] with 19-quarter GFC lead and
    95.7\% hit rate.  Nigeria's banking sector contributes the highest
    regional AUROC (0.893), reflecting sharp structural breaks around
    the 2009 consolidation reforms.  The $\delta_C$ (credit camouflage)
    channel dominates across all non-US regions; ING is the sole
    institution-level outlier at $+4.60\sigma$.
\end{enumerate}
...
\paragraph{Panel coverage.}
The institution-level panels ($N=20$ U.S.\ banks, $N=24$ global institutions
across 5~regions) exclude banks with $< 25\%$ data coverage.  The
global panel uses country-level World Bank indicators rather than
bank-level data for non-US institutions, meaning all banks from the
same country share identical features.  This inflates cross-country
Mahalanobis distance and makes per-region signal magnitudes
non-comparable without normalisation.  Nigerian banks (added as an
emerging-market benchmark) use World Bank data only, with no ECB
or FDIC granularity.

\paragraph{Granger causality.}
The universal Granger failure, while theoretically consistent with the
phase-transition thesis, means that the model cannot be validated by
standard linear forecasting metrics.  Developing nonlinear or threshold-based
predictability tests that align with the theoretical framework is an important
next step.
...
\begin{enumerate}
  ...
  \item \textbf{Tighter separation bounds.}  Proposition~\ref{prop:separation} derives
        $\nu > 0$ from the condition number $\kappa(\Sigma_0)$ of the reference
        covariance.  The current bound $\nu_C \ge \kappa(\Sigma_0)^{-1}$ is
        conservative; tighter bounds incorporating the distribution of
        eigenvalues (not just extremes) and the interaction between operators
        could yield sharper equivalence constants $c_1, c_2$ in Theorem~C.

  \item \textbf{Nonlinear forecasting tests.}  Replace Granger causality with
        tests designed for threshold/phase-transition processes: exceedance
        regression~\citep{allen2012systemic}, quantile causality, or
        time-varying parameter models that can detect state-dependent
        predictive power.
...
The economic interpretation (Section~\ref{sec:economic}) distils these technical
findings into three actionable principles: (i)~low volatility should be treated
as a warning, not a reassurance; (ii)~monitoring frameworks must be
regionally calibrated to the architecture of each financial system (bank-based
vs.\ market-based); and (iii)~stress-test scenarios should include structurally
novel crisis types, not only replays of past events.  The full guidance for
central banks, commercial banks, and investors is provided in
Sections~\ref{sec:advice_cb}--\ref{sec:advice_investor}.
...
The pipeline can be reproduced with:
\begin{verbatim}
  cd adaptive-friction-stability-upgraded/pipeline
