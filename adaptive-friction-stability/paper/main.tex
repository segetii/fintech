...
features) across three crises---2008~GFC, 2020~COVID, 2022~rate shock.
We adopt a rigorous evaluation protocol aligned with phase-transition dynamics:
the MFLS signal achieves AUROC $\geq 0.70$ over the full 34-year panel, fires its
GFC alarm 6~quarters before Lehman with a 71.4\% hit rate using only a calm-period
threshold, and passes three pre-registered robustness checks (alternate normal period,
rolling refit, leave-one-crisis-out threshold calibration).
We report Granger causality results honestly: all five MFLS variants fail at
$\alpha = 0.10$, confirming the phase-transition thesis---crises are threshold
crossings, not linearly predictable trends.  The Signed~LR variant discovers that
...
\begin{enumerate}
  ...
  \item \textbf{Empirical validation} (Section~\ref{sec:empirical}): on 140~quarters
        of real FDIC SDI data ($N=7$, $d=6$), the MFLS alarm leads the GFC by
        6~quarters; five independently constructed scoring variants all confirm
        the phase-transition thesis (universal Granger failure); a comparison
        table (Table~\ref{tab:indicator_comparison}) shows that the indicators
        which \emph{passed} Granger tests pre-2008 all failed in real time; and
        a learned variant discovers convergent herding as a crisis precursor.
\end{enumerate}
